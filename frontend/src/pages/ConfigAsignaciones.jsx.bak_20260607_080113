...
export default function ConfigAsignaciones() {
  ...
  const fetchUnidadesNegocio = async () => {
    try {
      ...
      setUnidadesNegocio(response.data.data || []);
    } catch (error) {
  ...
  const fetchAlmacenesUnidad = async (unidadId) => {
    try {
      ...
      return (response.data.data || []).filter(a => a.id);
    } catch (error) {
  ...
  const fetchUsuarios = async () => {
    try {
      ...
      const activos = (response.data.users || response.data || []).filter(u => u.activo !== false);
      setUsuarios(activos);
    } catch (error) {
        ...
        <CardContent className="pt-6">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex-1 flex flex-wrap items-center gap-3">
              ...
              {/* Filtro unidad */}
              <Select value={filtroUnidad || '__todas__'} onValueChange={(v) => setFiltroUnidad(v === '__todas__' ? '' : v)}>
                <SelectTrigger
