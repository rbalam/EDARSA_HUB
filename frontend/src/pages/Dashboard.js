...
import { getDiferenciaClass, getCostoDiferenciaClass, getAlmacenStatusBadge } from '../utils/styleHelpers';
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-3 border border-zinc-200 rounded-lg shadow-lg">
        ...
        {payload.map((entry) => (
          <p key={`${entry.name}-${entry.dataKey || entry.value}`} style={{ color: entry.color }} className="text-sm">
...
const Dashboard = () => {
  ...
  // MIGRACIÓN: Usar Unidades de Negocio en lugar de Servidores
  ...
  const loadUnidades = async () => {
    ...
    try {
      ...
      logger.log(`[Dashboard] Cargadas ${unidades.length} unidades de negocio`);
      
      // Auto-seleccionar si solo hay una unidad
      if (unidades.length === 1) {
        setSelectedUnidad(unidades[0].id);
      }
    } catch (error) {
      logger.error('Error cargando unidades de negocio:', error);
      setError('Error al cargar unidades de negocio');
    } finally {
  ...
  // MIGRACIÓN: Loading de unidades de negocio
  if (loadingUnidades) {
    return (
      <div className="flex items-center justify-center h-96" data-testid="dashboard-loading">
        <div className="text-center">
          ...
          <p className="text-zinc-600">Cargando unidades de negocio...</p>
        </div>
      </div>
    );
  }

  // MIGRACIÓN: Sin unidades de negocio
    ...
    return (
      <div className="space-y-6" data-testid="dashboard-page">
        <div>
          ...
          <p className="text-zinc-600 mt-1">Análisis de diferencias de inventario</p>
        </div>
        
        <Card className="border border-orange-200 bg-orange-50">
          <CardContent className="p-8 text-center">
            <AlertTriangle className="h-12 w-12 text-orange-500 mx-auto mb-4" />
            <h2 className="text-xl font-semibold text-orange-800 mb-2">No hay unidades de negocio disponibles</h2>
            <p className="text-orange-700 mb-4">
              No tienes acceso a ninguna unidad de negocio o no se han configurado correctamente.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  // MIGRACIÓN: Sin unidad seleccionada - mostrar vista inicial
  if (!selectedUnidad) {
    return (
      <div className="space-y-6" data-testid="dashboard-page">
        <div>
        ...
        {/* MIGRACIÓN: Selector de Unidad de Negocio */}
          ...
          <CardContent>
            <div className="max-w-md">
              <Select value={selectedUnidad} onValueChange={setSelectedUnidad}>
                <SelectTrigger data-testid="unidad-select">
                  <SelectValue placeholder="Selecciona una unidad para ver los datos" />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-sm text-zinc-500 mt-2">
                Selecciona una unidad de negocio para cargar los datos del dashboard
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-zinc-200 bg-zinc-50">
          <CardContent className="p-12 text-center">
            ...
            <h2 className="text-xl font-semibold text-zinc-600 mb-2">Selecciona una unidad de negocio</h2>
            <p className="text-zinc-500">
              Los gráficos y datos se mostrarán aquí una vez que selecciones una unidad
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="text-center sm:text-left flex-1">
          ...
          <p className="text-sm text-zinc-500">
            Análisis de diferencias - {unidadesNegocio.find(u => u.id === selectedUnidad)?.nombre || 'Selecciona una unidad'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          {/* MIGRACIÓN: Selector de Unidad de Negocio */}
          <Select value={selectedUnidad} onValueChange={setSelectedUnidad}>
            <SelectTrigger className="w-[250px]" data-testid="unidad-selector">
            ...
            <ResponsiveContainer width="100%" height={350}>
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                >
                  {resumenGrupoData.map((entry) => (
                    <Cell key={`cell-${entry.name}`} fill={COLORS[resumenGrupoData.indexOf(entry) % COLORS.length]} />
                  ))}
                </Pie>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {dashboardData?.resumen_por_almacen?.map((item) => (
                  <tr key={`almacen-${item.idalmacen || item.almacen}`} className="border-b border-zinc-100 hover:bg-zinc-50">
                    ...
                    <td className="p-3 text-right font-mono">
                      <span className={getDiferenciaClass(item.total_diferencia)}>
                        {formatNumber(item.total_diferencia)}
                      </span>
                    </td>
                    <td className="p-3 text-right font-mono">
                      <span className={getCostoDiferenciaClass(item.total_costo_diferencia)}>
                        {formatCurrency(item.total_costo_diferencia)}
                      </span>
                    </td>
                    <td className="p-3 text-center">
                      {(() => {
                        const status = getAlmacenStatusBadge(item.total_costo_diferencia);
