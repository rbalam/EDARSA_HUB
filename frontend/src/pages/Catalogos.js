...
const Catalogos = () => {
  ...
  const cargarDominios = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/api/catalogos/dominios`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      
      if (!response.ok) throw new Error('Error cargando dominios');
  ...
  const cargarRegistros = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/catalogos/tabla/${catalogoActivo.tabla}?${params}`,
        { headers: { Authorization: `Bearer ${getToken()}` } }
      );
  ...
  const guardarRegistro = async () => {
    ...
    try {
      ...
      const url = modoEdicion
        ? `${API_URL}/api/catalogos/tabla/${catalogoActivo.tabla}/${registroEditando[estructura.pk]}`
        : `${API_URL}/api/catalogos/tabla/${catalogoActivo.tabla}`;
  ...
  const toggleActivoRegistro = async (registro, activar) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/catalogos/tabla/${catalogoActivo.tabla}/${registro[estructura.pk]}/${accion}`,
        {
          method: 'PUT',
          headers: { Authorization: `Bearer ${getToken()}` },
        }
      ...
      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.detail || `Error al ${accion}`);
      }
  ...
  const verificarTablas = async () => {
    try {
      const response = await fetch(`${API_URL}/api/catalogos/admin/verificar-tablas`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      
      if (!response.ok) throw new Error('Error verificando tablas');
      
      const data = await response.json();
      setEstadoTablas(data.tablas || {});
    } catch (error) {
      console.error('Error:', error);
  ...
  const crearTablasNuevas = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/api/catalogos/admin/crear-tablas`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      ...
      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.detail || 'Error creando tablas');
      }
      
      const data = await response.json();
      toast.success(`Tablas creadas: ${data.exitosos} exitosas, ${data.errores} errores`);
      verificarTablas();
      cargarDominios();
    } catch (error) {
      console.error('Error:', error);
