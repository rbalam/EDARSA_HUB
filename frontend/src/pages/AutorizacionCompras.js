...
export default function AutorizacionCompras() {
  ...
  const buscarFolioManual = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/api/compras/detalle-pedido-manual/${selectedServer}?folio=${encodeURIComponent(folioManual)}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const verDetalleMovimientos = async (codigo, producto) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/api/compras/detalle-movimientos/${selectedServer}?codigo_producto=${codigo}&almacenes=${infoInventario.almacenCodigos.join(',')}&fecha_ini=${fechaInvFisico}&fecha_fin=${fechaFinPeriodo}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const verDetalleConsumos = async (codigo, producto) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`${API_URL}/api/compras/detalle-consumos/${selectedServer}?codigo_producto=${codigo}&sucursal_codigo=${infoInventario.sucursalCodigo}&fecha_ini=${fechaInvFisico}&fecha_fin=${fechaFinPeriodo}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setDetalleModal(prev => ({ ...prev, data: response.data, loading: false }));
    } catch (error) {
      ...
      <Card className="border border-zinc-200 shadow-sm">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="space-y-1">
              ...
              <Select value={selectedServer} onValueChange={setSelectedServer}>
                <SelectTrigger data-testid="server-select" className="h-9">
          ...
          <div className="flex gap-2 pt-2">
            <Button onClick={calcularPedido} disabled={loading || !selectedServer || !selectedSucursal || (selectedAlmacenes.length === 0 && !todosAlmacenes)} data-testid="btn-calcular">
              ...
              <table className="w-full text-xs">
                ...
                <tbody>
                  {pedidoData.map((row, idx) => {
                    ...
                    return (
                      <tr key={idx} className={`border-b hover:bg-zinc-50 ${row.Sin_Inventario_Inicial ? 'bg-orange-50' : ''} ${stockBajo ? 'bg-red-50' : ''}`}>
                        ...
                        <td className="py-1 px-1 text-right cursor-pointer hover:text-blue-600" onDoubleClick={() => verDetalleMovimientos(row.Codigo, row.Producto)}>
                          <span className={row.Movimientos_Periodo > 0 ? 'text-green-700' : row.Movimientos_Periodo < 0 ? 'text-red-600' : ''}>{formatNumber(row.Movimientos_Periodo)}</span>
                        </td>
                        ...
                        {infoInventario?.comparandoConPedido && <td className={`py-1 px-1 text-right font-semibold ${row.Diferencia_Pedido > 0 ? 'text-green-600' : row.Diferencia_Pedido < 0 ? 'text-red-600' : ''}`}>{row.Diferencia_Pedido !== null ? (row.Diferencia_Pedido > 0 ? '+' : '') + formatNumber(row.Diferencia_Pedido) : '-'}</td>}
                      </tr>
                    );
                  })}
                </tbody>
