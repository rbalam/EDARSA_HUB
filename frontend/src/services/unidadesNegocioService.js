...
export const fetchUnidadesNegocio = async () => {
  ...
  if (!token) {
    console.warn('unidadesNegocioService: No hay token disponible');
    return [];
  }
  
  try {
    const response = await axios.get(`${API_URL}/api/unidades-negocio`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    
    console.log(`[Unidades] Cargadas ${response.data.length} unidades de negocio`);
