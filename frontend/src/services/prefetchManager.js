...
const execute = async (key) => {
  ...
  if (state.activeCount >= CONFIG.maxConcurrent) {
    console.log(`[Prefetch] En cola: ${key} (max concurrent reached)`);
    return;
  }
  ...
  try {
    ...
    console.log(`[Prefetch] Iniciando: ${key}`);
    ...
    console.log(`[Prefetch] Completado: ${key}`);
  } catch (error) {
    task.status = 'failed';
    state.activeCount--;
    state.failedCount++;
    console.warn(`[Prefetch] Error: ${key}`, error);
  }
};
