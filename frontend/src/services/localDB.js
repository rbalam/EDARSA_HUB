...
const set = async (storeName, key, data, ttlMinutes = 30) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(`[LocalDB] Guardado: ${storeName}/${key}`);
      resolve(true);
    };

    request.onerror = () => {
      console.error(`[LocalDB] Error guardando: ${storeName}/${key}`, request.error);
      reject(request.error);
    };
  });
};
    ...
    request.onsuccess = () => {
      ...
      if (!ignoreExpiry && record.expires_at) {
        ...
        if (expiresAt < new Date()) {
          console.log(`[LocalDB] Expirado: ${storeName}/${key}`);
          resolve(null);
          return;
        }
      }

      console.log(`[LocalDB] Leído: ${storeName}/${key}`);
      resolve(record);
    };

    request.onerror = () => {
      console.error(`[LocalDB] Error leyendo: ${storeName}/${key}`, request.error);
      reject(request.error);
    };
  });
};
...
const remove = async (storeName, key) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(`[LocalDB] Eliminado: ${storeName}/${key}`);
      resolve(true);
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = (event) => {
      ...
      if (cursor) {
        ...
        console.log(`[LocalDB] Limpieza ${storeName}: ${deleted} registros eliminados`);
        resolve(deleted);
      }
    };
...
const clearStore = async (storeName) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(`[LocalDB] Store limpiado: ${storeName}`);
      resolve(true);
    };
