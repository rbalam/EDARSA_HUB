...
export default function PropinasTPV() {
  ...
  // Datos auxiliares - SUBFASE 3.5: Unidades desde EDARSAHUB
  ...
  const cargarUnidadesNegocio = async () => {
    ...
    try {
      // Primero intentar endpoint v2 EDARSAHUB
      ...
      // Fallback a endpoint legacy de servers
  ...
  const cargarDatosAuxiliares = async () => {
    try {
      ...
      if (resServers.ok) {
        ...
        (data || []).forEach(s => { if (s.empresa) empresasSet.add(s.empresa); });
        setEmpresas(Array.from(empresasSet));
      }
    } catch (error) {
      logger.error('Error cargando datos auxiliares:', error);
    }
  };
