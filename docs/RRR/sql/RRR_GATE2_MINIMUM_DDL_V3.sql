/*
RRR GATE 2 - MINIMUM DDL V3 - FINAL DESIGN CANDIDATE
DESIGN ONLY. NO EJECUTAR DESDE ESTE JOB. Production=false.

Contratos externos certificados:
- ClienteID int -> dbo.Cliente_Catalogo(ClienteID)
- EmpresaID int -> dbo.Sistema_Empresas(EmpresaID)
- UnidadNegocioID uniqueidentifier -> dbo.Unidades_Negocio(id)
- VentaID bigint -> dbo.Venta_Encabezado(VentaID)
- UsuarioID int -> dbo.Usuario_Catalogo(UsuarioID)

Sucursal fisica es un dominio distinto: dbo.Sistema_Sucursales(SucursalID int).
RRR_Eventos usa UnidadNegocioID como identidad canonica de unidad de negocio.
*/
...
CREATE TABLE dbo.RRR_Reglas (
    ...
    CONSTRAINT FK_RRR_Reglas_UsuarioCreacion FOREIGN KEY (UsuarioCreacionID) REFERENCES dbo.Usuario_Catalogo(UsuarioID)
);
...
CREATE TABLE dbo.RRR_ReglasVersiones (
    ...
    CONSTRAINT FK_RRR_ReglasVersiones_UsuarioCreacion FOREIGN KEY (UsuarioCreacionID) REFERENCES dbo.Usuario_Catalogo(UsuarioID),
...
CREATE TABLE dbo.RRR_Eventos (
    ...
    CONSTRAINT FK_RRR_Eventos_Empresa FOREIGN KEY (EmpresaID) REFERENCES dbo.Sistema_Empresas(EmpresaID),
    CONSTRAINT FK_RRR_Eventos_UnidadNegocio FOREIGN KEY (UnidadNegocioID) REFERENCES dbo.Unidades_Negocio(id),
    CONSTRAINT FK_RRR_Eventos_Venta FOREIGN KEY (VentaID) REFERENCES dbo.Venta_Encabezado(VentaID)
);
...
CREATE TABLE dbo.RRR_Beneficios (
    ...
    CONSTRAINT FK_RRR_Beneficios_UsuarioCreacion FOREIGN KEY (UsuarioCreacionID) REFERENCES dbo.Usuario_Catalogo(UsuarioID)
);
...
CREATE TABLE dbo.RRR_LedgerMovimientos (
    ...
    CONSTRAINT FK_RRR_Ledger_UsuarioCreacion FOREIGN KEY (UsuarioCreacionID) REFERENCES dbo.Usuario_Catalogo(UsuarioID),
...
/*
Vistas derivadas propuestas para gate posterior:
- dbo.vw_RRR_ClienteBalance: SUM neta del ledger valido.
- dbo.vw_RRR_ClienteEstadoActual: ultimo score + rank + balance.

No se crea tabla mutable de saldo.
No se modifica CRM_PostventaEncuestas en Gate 2.
No se fusiona Cavas con RRR.
*/
