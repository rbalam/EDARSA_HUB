/*
CAVAS CORPORATIVAS - DDL MINIMO PROPUESTO
ESTADO: PROPUESTA SOLAMENTE - NO EJECUTAR
PRECONDICION: GATE 2 CERTIFIED / INTEGRATED / 100%
PRODUCTION: PROHIBIDA

Principios:
- No duplica Sistema_Empresas, Unidades_Negocio, CRM, Usuario_Catalogo, productos ni ventas/tickets.
- FKs fisicas solo entre tablas propias de Cavas Corporativas.
- Referencias a dominios canonicos externos permanecen desacopladas mientras no exista contrato de llave/tipo certificado.
- Opt-in: cualquier alcance no configurado explicitamente no recibe beneficio.
- Alimentos/bebidas/ambos/futuras lineas se modelan como LineaComercialCodigo, no como columnas rigidas.
- Horarios son configuracion, nunca hardcode.
*/

SET NOCOUNT ON;
SET XACT_ABORT ON;

/* 1. Referencia a organizacion cliente B2B sin copiar PII. */
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_CC_Convenio_Organizacion FOREIGN KEY (OrganizacionCorporativaID)
            REFERENCES dbo.CavasCorporativas_OrganizacionesRef(OrganizacionCorporativaID),
        CONSTRAINT CK_CC_Convenio_Vigencia CHECK (VigenciaHasta IS NULL OR VigenciaHasta >= VigenciaDesde)
    );
END;

/* 3. Unidades participantes. La llave externa se conserva textual hasta certificar tipo canonico exacto. */
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_CC_ConvenioUnidad_Convenio FOREIGN KEY (ConvenioID)
            REFERENCES dbo.CavasCorporativas_Convenios(ConvenioID)
    );
END;

/* 4. Personas/usuarios autorizados. No copia nombre, email ni PII. */
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_CC_Beneficio_Convenio FOREIGN KEY (ConvenioID)
            REFERENCES dbo.CavasCorporativas_Convenios(ConvenioID),
...
/* 6. Alcance opt-in. Lo que no aparece aqui queda fuera del beneficio. */
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_CC_Alcance_Beneficio FOREIGN KEY (BeneficioID)
            REFERENCES dbo.CavasCorporativas_Beneficios(BeneficioID),
        CONSTRAINT CK_CC_Alcance_Nivel CHECK (NivelDetalle IN ('LINEA','CATEGORIA','FAMILIA','SKU'))
    );
    CREATE UNIQUE INDEX UX_CC_Alcance
        ON dbo.CavasCorporativas_BeneficioAlcances
        (BeneficioID, UnidadReferencia, LineaComercialCodigo, NivelDetalle, ReferenciaDetalle);
END;

/* 7. Politicas/restricciones configurables. */
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_CC_Politica_Convenio FOREIGN KEY (ConvenioID)
            REFERENCES dbo.CavasCorporativas_Convenios(ConvenioID),
        CONSTRAINT FK_CC_Politica_Beneficio FOREIGN KEY (BeneficioID)
            REFERENCES dbo.CavasCorporativas_Beneficios(BeneficioID),
        CONSTRAINT CK_CC_Politica_ConfigJson CHECK (ISJSON(ConfiguracionJson)=1)
    );
END;

/* 8. Aplicacion auditable del beneficio a una operacion externa/canonica. */
...
BEGIN
    ...
    (
        ...
        CONSTRAINT FK_CC_Aplicacion_Convenio FOREIGN KEY (ConvenioID)
            REFERENCES dbo.CavasCorporativas_Convenios(ConvenioID),
        CONSTRAINT FK_CC_Aplicacion_Beneficio FOREIGN KEY (BeneficioID)
            REFERENCES dbo.CavasCorporativas_Beneficios(BeneficioID),
        CONSTRAINT FK_CC_Aplicacion_Autorizado FOREIGN KEY (AutorizadoID)
            REFERENCES dbo.CavasCorporativas_Autorizados(AutorizadoID),
        CONSTRAINT UQ_CC_Aplicacion_Idempotencia UNIQUE
            (BeneficioID, UnidadReferencia, OperacionTipo, OperacionReferencia),
        CONSTRAINT CK_CC_Aplicacion_EvaluacionJson CHECK (EvaluacionJson IS NULL OR ISJSON(EvaluacionJson)=1)
    );
END;

/* Indices minimos operativos. */
...
/*
PENDIENTES ANTES DE CONVERTIR EN MIGRACION EJECUTABLE:
1. Certificar entidad/llave canonica para organizacion cliente B2B; reemplazar FuenteCanonica/ReferenciaCanonica por FK solo si corresponde.
2. Certificar tipo exacto de `unidad_negocio_pk`; reemplazar UnidadReferencia por FK a dbo.Unidades_Negocio solo despues de esa evidencia.
3. Certificar identificadores de linea/categoria/familia/SKU para valorar FKs en Alcances.
4. Certificar contrato canonico de reservas/tickets/consumos y evento temporal gobernante para el limite antes de 22:00.
5. Definir catalogo controlado de TipoBeneficio/TipoPolitica/OperacionTipo sin hardcode de reglas comerciales.
6. Agregar permisos RBAC especificos del modulo en una migracion separada.
*/
