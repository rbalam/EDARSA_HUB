...
@router.get(
    "/perfil-unidad/{unidad_negocio_id}",
...
async def obtener_perfil(
    ...
    if not perfil:
        raise HTTPException(
            status_code=404,
            detail=f"No existe perfil digital para la unidad {unidad_negocio_id}"
        )
...
async def crear_perfil(
    ...
    try:
        ...
        logger.info(f"[API] Perfil digital creado para unidad {data.unidad_negocio_id}")
        return perfil
...
@router.get(
    "/benchmark/resumen/{unidad_negocio_id}",
...
async def obtener_resumen(
    ...
    NO ejecuta IA.
    """
    resumen = obtener_resumen_benchmark(unidad_negocio_id, empresa_id)
    return resumen


@router.get(
    "/benchmark/estado-preparacion/{unidad_negocio_id}",
...
async def obtener_estado_preparacion(
    ...
    """
    estado = obtener_estado_preparacion_ia(unidad_negocio_id, empresa_id)
    return estado
