...
# - /comercial/tablero-ejecutivo - Usa vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
# ============================================================================
...
async def comercial_detalle_movimientos(
    ...
    Este endpoint ahora lee de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime en EDARSAHUB SQL.
    ...
    try:
        ...
        # Query para obtener totales (columnas reales de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime)
        query_total = f"""
        SELECT
...
async def comercial_dashboard(
    ...
    try:
        ...
        if is_softrestaurant_system(server.get('system_type')):
            ...
            # PERO EDARSAHUB sí tiene datos en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime.
            #
...
def _get_edarsahub_config_dict() -> dict:
    cfg = get_edarsahub_sql_config()
    return {'host': cfg.host, 'port': cfg.port, 'database': cfg.database, 'username': cfg.user, 'password': cfg.password}
...
async def ventas_tiempo(unit_id: str, current_user: Dict = Depends(get_current_user)):
    ...
    base_hourly = 20000
    if UnidadesService.resolver_codigo("CIENFUEGOS") or "CIENFUEGOS" in clean_id:
        base_hourly = 24000
    elif any(x in clean_id for x in ["MERIDA", UnidadesService.resolver_codigo("130MID") or "130MID", "MID"]):
        base_hourly = 21000
    elif any(x in clean_id for x in ["QUERETARO", UnidadesService.resolver_codigo("130QRO") or "130QRO", "QRO"]):
        base_hourly = 19000
    elif UnidadesService.resolver_codigo("ESTELAR") or "ESTELAR" in clean_id:
        base_hourly = 15000
    elif UnidadesService.resolver_codigo("ORIGEN") or "ORIGEN" in clean_id:
        base_hourly = 12000
