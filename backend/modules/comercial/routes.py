...
# FASE CACHE: Import de funciones de cache
...
from modules.comercial_v2.repository_readonly import get_ventas_dia_abiertas, get_kpis_diarios
...
# - /comercial/tablero-ejecutivo - Usa vw_Comercial_KPIs_Diarios_v2_Runtime
# ============================================================================
...
def get_ventas_dia_snapshot_from_edarsahub(server_id: str, fecha_operacion: str = None, unidad_negocio_pk: str = None) -> Dict:
    """
    ...
    Args:
        ...
        unidad_negocio_pk: ID de la unidad para calcular FechaOperacion (opcional, pero recomendado)
    ...
    if fecha_operacion is None:
        if unidad_negocio_pk:
            ...
            fecha_op_calc, hora_ini, hora_fin, cruza = get_operational_window(unidad_negocio_pk, now_mx)
            fecha_operacion = fecha_op_calc.isoformat()
            logging.info(
                f"[EDARSAHUB-SNAPSHOT] {unidad_negocio_pk}: FechaOperacion calculada = {fecha_operacion} "
...
async def _tablero_ejecutivo_internal(
                        ...
                        if data_type == "EDARSAHUB_VENTAS_DIA":
                            ...
                            for vd in ventas_dia_snapshot:
                                ...
                                if vd.get('server_id') == server['id']:
                                    servidor_data = vd
                                    break
                                # Criterio 2: Match por unidad_negocio_pk
                                if unidad_codigo and vd.get('unidad_negocio_pk') == unidad_codigo:
                                    servidor_data = vd
                                    break
            ...
            # FASE 3A.2: Migrado a helper centralizado
                ...
                if data_type == "EDARSAHUB_VENTAS_DIA":
                    ...
                    try:
                        ...
                        # Mapeo canónico servidor → unidad_negocio_pk
                        if UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN' in server_name_upper:
                            unidad_codigo_mpro = UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN'
                        elif 'QRO' in server_name_upper or 'QUERETARO' in server_name_upper:
                            unidad_codigo_mpro = UnidadesService.resolver_codigo('130QRO') or '130QRO'
                        else:
                            unidad_codigo_mpro = server.get('unidad_negocio_pk') or server_name_upper.replace(' LOCAL', '').strip()
                        ...
                        server_name_lower = server.get('name', '').lower().strip()
                        for vd in ventas_dia_snapshot:
                            vd_id = vd.get('unidad_negocio_pk', '')
                            vd_nombre = (vd.get('unidad_negocio_nombre') or '').lower().strip()
                    ...
                    # MPRO modo HUB: leer exclusivamente desde EDARSAHUB SQL canonico.
                    # No API local, no SQL directo a MPRO, no cache legacy, no PAX inventado.
                    try:
                        periodo_ini_dt = datetime.strptime(fecha_ini, "%Y-%m-%d").date()
                        periodo_fin_dt = datetime.strptime(fecha_fin, "%Y-%m-%d").date()
                        periodo_ant_ini_dt = datetime.strptime(fecha_ini_ant, "%Y-%m-%d").date()
                        periodo_ant_fin_dt = datetime.strptime(fecha_fin_ant, "%Y-%m-%d").date()
                        periodo_anio_ini_dt = datetime.strptime(fecha_ini_año_ant, "%Y-%m-%d").date()
                        periodo_anio_fin_dt = datetime.strptime(fecha_fin_año_ant, "%Y-%m-%d").date()

                        server_id_actual = str(server.get('id') or '')

                        def _filtrar_server(rows):
                            return [
                                row for row in rows
                                if str(row.get('server_id') or '') == server_id_actual
                            ]

                        def _agrupar_por_unidad(rows):
                            agrupado = {}
                            for row in rows:
                                unidad_pk = str(
                                ...
                                item = agrupado.setdefault(unidad_pk, {
                                    'unidad_negocio_pk': unidad_pk,
                                    'unidad_negocio_codigo': row.get('unidad_negocio_id') or unidad_pk,
                                    'unidad_negocio_nombre': row.get('unidad_negocio_nombre') or unidad_pk,
                                    'unidad': row.get('unidad_negocio_nombre') or unidad_pk,
                                    'sistema_origen': row.get('sistema_origen') or 'MPRO',
                                    'ventas': 0.0,
                                    'pax': 0,
                                    'cheques': 0,
                                    '_rows': 0,
                        ...
                        rows_actual = _filtrar_server(get_kpis_diarios(periodo_ini_dt, periodo_fin_dt, None))
                        rows_ant = _filtrar_server(get_kpis_diarios(periodo_ant_ini_dt, periodo_ant_fin_dt, None))
                        rows_anio = _filtrar_server(get_kpis_diarios(periodo_anio_ini_dt, periodo_anio_fin_dt, None))

                        actual_por_unidad = _agrupar_por_unidad(rows_actual)
                        ant_por_unidad = _agrupar_por_unidad(rows_ant)
                        anio_por_unidad = _agrupar_por_unidad(rows_anio)

                        unidades_keys = sorted(
                            set(actual_por_unidad.keys())
                            | set(ant_por_unidad.keys())
                            | set(anio_por_unidad.keys())
                        )

                        unidades_canonicas = []
                        for unidad_key in unidades_keys:
                            actual = actual_por_unidad.get(unidad_key, {})
                            ant = ant_por_unidad.get(unidad_key, {})
                            anio_ref = anio_por_unidad.get(unidad_key, {})
                            base = actual or ant or anio_ref

                            ventas = float(actual.get('ventas') or 0)
                            cheques = int(actual.get('cheques') or 0)
                            pax = int(actual.get('pax') or 0)

                            kpis_mpro = {
                                'unidad': base.get('unidad') or unidad_key,
                                'unidad_negocio_pk': base.get('unidad_negocio_pk') or unidad_key,
                                'unidad_negocio_codigo': base.get('unidad_negocio_codigo') or unidad_key,
                                'unidad_negocio_nombre': base.get('unidad_negocio_nombre') or unidad_key,
                                'ventas': ventas,
                                'ventas_ant': float(ant.get('ventas') or 0),
                                'ventas_año': float(anio_ref.get('ventas') or 0),
                                'pax': pax,
                                'pax_ant': int(ant.get('pax') or 0),
                                'pax_año': int(anio_ref.get('pax') or 0),
                                'cheques': cheques,
                                'cheques_ant': int(ant.get('cheques') or 0),
                                'cheques_año': int(anio_ref.get('cheques') or 0),
                                'ticket_prom': round(ventas / cheques, 2) if cheques > 0 else 0,
                                'cheque_prom': round(ventas / cheques, 2) if cheques > 0 else 0,
                                'pax_prom': round(pax / cheques, 2) if cheques > 0 else 0,
                                'proyeccion': round((ventas / dias_transcurridos) * dias_mes, 2) if dias_transcurridos > 0 else ventas,
                                'origen': 'EDARSAHUB_SQL',
                                'fuente': 'EDARSAHUB_SQL',
                                '_rows_actual': int(actual.get('_rows') or 0),
                            }
                            unidades_canonicas.append(kpis_mpro)

                        unidades_canonicas = await filtrar_unidades_por_visibilidad(unidades_canonicas, server['id'])
                        logging.info(f"[MPRO-CANONICO] {server['name']}: {len(unidades_canonicas)} unidades desde EDARSAHUB SQL")

                        if not unidades_canonicas:
                            unit_response = build_unit_response(
                                server=server,
                                kpis={'ventas': 0, 'pax': 0, 'cheques': 0},
                                data_status=DataStatus.NO_DATA_CONFIRMED,
                                live_status=LiveStatus.LIVE_NOT_APPLICABLE,
                                cache_status=CacheStatus.MISSING,
                                source_used=SourceUsed.NONE,
                                ...
                                source_period="EDARSAHUB_SQL",
                                source_live="EDARSAHUB_SQL",
                                sucursal=server.get('name', 'MPRO'),
                            )
                            resultados.append(unit_response)
                        else:
                            for unidad in unidades_canonicas:
                                tiene_dato_actual = unidad.get('_rows_actual', 0) > 0

                                unit_response = build_unit_response(
                                    server=server,
                                    kpis=unidad,
                                    data_status=DataStatus.DATA_OK if tiene_dato_actual else DataStatus.NO_DATA_CONFIRMED,
                                    live_status=LiveStatus.LIVE_NOT_APPLICABLE,
                                    cache_status=CacheStatus.NOT_USED,
                                    source_used=SourceUsed.REAL_SOURCE if tiene_dato_actual else SourceUsed.NONE,
                                    source_real_attempted=True,
                                    source_real_status=SourceRealStatus.SUCCESS,
                                    source_period="EDARSAHUB_SQL",
                                    source_live="EDARSAHUB_SQL",
                                    sucursal=unidad.get('unidad', 'MPRO'),
                                )
                                resultados.append(unit_response)
                        ...
                        logging.error(f"[MPRO-CANONICO] Error EDARSAHUB SQL: {server['name']}: {mpro_error}")
                        unit_response = build_unit_response(
                            server=server,
                            kpis=None,
                            data_status=DataStatus.DATA_ERROR,
                            ...
                            source_period="EDARSAHUB_SQL",
                            source_live="EDARSAHUB_SQL",
                            error_code="MPRO_EDARSAHUB_SQL_ERROR",
                            error_message=str(mpro_error)[:100],
                        )
                        resultados.append(unit_response)
    ...
    totales["ticket_prom"] = round(totales["ventas"] / totales["cheques"], 2) if totales["cheques"] > 0 else 0
    ...
    if solo_ventas_dia:
        try:
            ...
            for r in resultados:
                uid = r.get('unidad_negocio_pk') or r.get('server_id')
                if uid:
            ...
            for snapshot in all_snapshots:
                uid = snapshot.get('unidad_negocio_pk', '')
...
"""
        ...
        # FASE 3A.2: Migrado a helper centralizado
            ...
            NOMBRE_A_CODIGO_SUCURSAL = {
                UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN': '0023',
                'QUERETARO': '0021',
                '130 QRO': '0021',
                'QRO': '0021',
            }
...
async def ventas_precios_constantes(
    ...
    """
    Precios constantes SQL-FIRST.

    FIX P1 2026-06-04:
    Lee exclusivamente desde Sync_Precios_Historicos en EDARSAHUB SQL.
    Prohibido conexión LIVE a SoftRestaurant/MPRO.
    """
    server = await get_server_by_id(server_id)
    if not server:
        raise HTTPException(status_code=404, detail="Servidor no encontrado")
    ...
    try:
        ...
        filtros = [f"ServerID = '{server_id}'"]
        if sucursal and sucursal.upper() not in ("DEFAULT", "ALL", "TODAS"):
            filtros.append(f"SucursalID = '{sucursal}'")
        if producto_id:
            filtros.append(f"ProductoID = '{producto_id}'")

        filtros.append(f"FechaVigencia <= '{fecha_fin}'")
        filtros.append(f"(FechaFinVigencia IS NULL OR FechaFinVigencia >= '{fecha_inicio}')")
        ...
        total_rows = _query_edarsahub_tablero(query_total)
        ...
        FROM Sync_Precios_Historicos
        WHERE {where_sql}
        ORDER BY ProductoNombre ASC, FechaVigencia DESC
        OFFSET {offset} ROWS FETCH NEXT {limit} ROWS ONLY
        """

        rows = _query_edarsahub_tablero(query_data)

        precios = []
        for r in rows or []:
            fv = r.get("FechaVigencia")
            ff = r.get("FechaFinVigencia")
            fs = r.get("FechaSync")

            precios.append({
                "precio_historico_id": r.get("PrecioHistoricoID"),
                "server_id": r.get("ServerID"),
                "sucursal_id": r.get("SucursalID"),
                "producto_id": r.get("ProductoID"),
                "producto_codigo": r.get("ProductoCodigo"),
                "producto_nombre": r.get("ProductoNombre"),
                "fecha_vigencia": fv.strftime("%Y-%m-%d") if hasattr(fv, "strftime") else str(fv) if fv else None,
                "fecha_fin_vigencia": ff.strftime("%Y-%m-%d") if hasattr(ff, "strftime") else str(ff) if ff else None,
                "precio_base": float(r.get("PrecioBase") or 0),
                "precio_final": float(r.get("PrecioFinal") or 0),
                "impuesto_incluido": bool(r.get("ImpuestoIncluido")) if r.get("ImpuestoIncluido") is not None else None,
                "tasa_impuesto": float(r.get("TasaImpuesto") or 0),
                "precio_anterior": float(r.get("PrecioAnterior") or 0) if r.get("PrecioAnterior") is not None else None,
                "variacion_porcentaje": float(r.get("VariacionPorcentaje") or 0) if r.get("VariacionPorcentaje") is not None else None,
                "motivos_cambio": r.get("MotivosCambio"),
                "usuario_modificacion": r.get("UsuarioModificacion"),
                "fecha_sync": fs.strftime("%Y-%m-%d %H:%M:%S") if hasattr(fs, "strftime") else str(fs) if fs else None
            })

        return {
            "source_status": "SUCCESS" if total > 0 else "NO_DATA",
            "source_message": "Precios constantes consultados desde EDARSAHUB SQL",
            "source": "EDARSAHUB_SQL_V2",
            "data": precios,
            "precios": precios,
            "total": total,
        ...
        logging.error("[PRECIOS_CONSTANTES_SQL_FIRST] Error: %s", e)
        return {
            "source_status": "ERROR",
            "source_message": str(e),
            "source": "EDARSAHUB_SQL_V2",
...
# ============================================================================
# NOTA: Código legacy de reporte PAX eliminado (líneas 4211-4622)
# El endpoint ahora usa exclusivamente SQL-First vía EDARSAHUB.
# ============================================================================
...
async def comercial_dashboard(
    ...
    try:
        ...
        if is_softrestaurant_system(server.get('system_type')):
            ...
            # PERO EDARSAHUB sí tiene datos en vw_Comercial_KPIs_Diarios_v2_Runtime.
            #
