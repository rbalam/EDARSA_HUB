...
async def comercial_detalle_movimientos(
    ...
    Este endpoint ahora lee de Comercial_KPIs_Diarios_v2 en EDARSAHUB SQL.
    ...
    try:
        ...
        unidad_ids = []
        if server.get("unidad_negocio_id"):
            unidad_ids.append(server.get("unidad_negocio_id"))
        ...
        if unidad_ids:
            ...
            filtro_principal = f"(unidad_negocio_id IN ({filtro_unidades}) OR server_id = '{server_id}')"
        else:
            filtro_principal = f"server_id = '{server_id}'"

        # Query para obtener totales (columnas reales de Comercial_KPIs_Diarios_v2)
        query_total = f"""
        SELECT
