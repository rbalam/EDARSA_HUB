...
def relacionar_competidor_unidad(
    ...
    if existe_result:
        return {
            ...
            "mensaje": f"El competidor '{nombre_competidor}' ya está relacionado con la unidad {unidad_negocio_pk}"
        }
    ...
    logger.info(f"[COMPETIDOR_UNIDAD] Relacionado {nombre_competidor} -> Unidad {unidad_negocio_pk}")
    ...
    return {
        ...
        "mensaje": f"Competidor '{nombre_competidor}' relacionado con unidad {unidad_negocio_pk}"
    }
...
def desrelacionar_competidor_unidad(
    ...
    logger.info(f"[COMPETIDOR_UNIDAD] Desrelacionado {competidor_catalogo_id} de Unidad {unidad_negocio_pk}")
    
    return {
        "mensaje": f"Competidor desrelacionado de unidad {unidad_negocio_pk}",
        "competidor_catalogo_id": competidor_catalogo_id,
        "unidad_negocio_pk": unidad_negocio_pk
    }
...
def listar_competidores_por_unidad(
    ...
    SIEMPRE debe recibir unidad_negocio_pk.
    """
    conn = _get_conn()
    
    where_clauses = [f"UnidadNegocioID = {unidad_negocio_pk}"]
    ...
    for row in result:
        competidores.append({
            ...
            "unidad_negocio_pk": row.get('UnidadNegocioID'),
            "nombre_competidor": row.get('NombreCompetidor'),
            "tipo_restaurante": row.get('TipoRestaurante'),
            "segmento_precio": row.get('SegmentoPrecio'),
            "ciudad": row.get('Ciudad'),
...
def estadisticas_competidores_unidad(unidad_negocio_pk: int) -> Dict[str, Any]:
    """
...
def validar_acceso_unidad(usuario_id: str, unidad_negocio_pk: int, es_superadmin: bool = False) -> bool:
    """
    ...
    logger.warning(f"[RBAC] Validación de acceso pendiente para usuario {usuario_id} -> unidad {unidad_negocio_pk}")
    return True
