...
def listar_perfiles_digitales(
    ...
    if unidad_negocio_pk:
        where_clauses.append(f"UnidadNegocioID = {unidad_negocio_pk}")
    if solo_activos:
...
def obtener_perfil_por_unidad(unidad_negocio_pk: int) -> Optional[PerfilDigitalResponse]:
    """
...
def crear_perfil_digital(data: PerfilDigitalCreate, usuario: str) -> PerfilDigitalResponse:
    ...
    if check_result and check_result[0]['existe'] > 0:
        raise ValueError(f"Ya existe un perfil digital activo para la unidad {data.unidad_negocio_pk}")
    ...
    logger.info(f"[PERFIL-DIGITAL] Creado perfil {perfil_id} para unidad {data.unidad_negocio_pk} por {usuario}")
...
def obtener_urls_perfil(unidad_negocio_pk: int) -> Dict[str, Optional[str]]:
    """
...
def verificar_perfil_completo(unidad_negocio_pk: int) -> Dict[str, Any]:
    """
