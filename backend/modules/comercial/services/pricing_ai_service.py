...
def _obtener_contexto_unidad(unidad_negocio_pk: int) -> Dict[str, Any]:
    """
...
async def analizar_producto_con_ia(
    ...
    contexto_unidad = _obtener_contexto_unidad(unidad_negocio_pk)
    
    # 3. Obtener competidores y benchmark (filtrando por lista si aplica)
    competidores = _obtener_competidores_para_contexto(unidad_negocio_pk, limit=5, lista_id=lista_id)
...
async def sugerir_comparables_con_ia(
    ...
    competidores = _obtener_competidores_para_contexto(unidad_negocio_pk, limit=10)
...
{{
  ...
  "areas_revisar": ["área 1", "área 2"]
    ...
    try:
        ...
        chat = LlmChat(
            api_key=llm_key,
            session_id=f"benchmark-{unidad_negocio_pk}-{uuid.uuid4().hex[:8]}",
