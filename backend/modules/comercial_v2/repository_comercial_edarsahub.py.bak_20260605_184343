...
# =============================================================================
# OPERACIONES EN vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime
# =============================================================================

def upsert_kpi_diario(kpi: KPIsDiariosV2) -> Dict[str, Any]:
    """
    Inserta o actualiza un KPI diario en vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime.
    
    Lógica:
    1. Busca registro existente por (unidad_negocio_pk, sucursal_id, fecha_operacion)
    ...
    FROM vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime
    ...
    if existing:
        ...
        if record.get('hash_origen') == kpi.hash_origen:
            ...
            update_query = f"""
            UPDATE vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime SET
        ...
        insert_query = f"""
        INSERT INTO vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime (
...
def upsert_ventas_dia_abiertas(ventas: VentasDiaAbiertasV2) -> Dict[str, Any]:
    ...
    if not ventas.sync_run_id or not ventas.sync_run_id.startswith("ABIERTA-"):
        logger.error(
            ...
            f"unidad={ventas.unidad_negocio_pk}, run_id={ventas.sync_run_id}, "
    ...
    try:
        ...
        if str(fecha_recibida) != str(fecha_correcta):
            logger.error(
                ...
                f"unidad={ventas.unidad_negocio_pk}, "
    ...
    if ventas.fecha_operacion > fecha_hoy_mx:
        logger.error(
            ...
            f"fecha_hoy={fecha_hoy_mx}. Unidad={ventas.unidad_negocio_pk}, run_id={ventas.sync_run_id}"
        )
        return {
    ...
    if existing:
        ...
        if existing_fecha != new_fecha:
            logger.warning(
                f"[UPSERT-FECHA] {ventas.unidad_negocio_pk}: Actualizando fecha "
        ...
        """
        _execute_query(update_query)
        logger.info(f"[UPSERT] UPDATE {ventas.unidad_negocio_pk}: ${ventas.total_estimado_dia:,.2f}, fecha={ventas.fecha_operacion}")
        ...
        """
        _execute_query(insert_query)
        logger.info(f"[UPSERT] INSERT {ventas.unidad_negocio_pk}: ${ventas.total_estimado_dia:,.2f}")
        return {'action': 'INSERT', 'id': new_id}
...
def insert_sync_log(log: SyncLogV2) -> str:
    ...
    INSERT INTO Comercial_SyncLog_v2 (
        ...
        SYSUTCDATETIME(),
        '{log.run_type}',
        {f"'{log.unidad_negocio_pk}'" if log.unidad_negocio_pk else 'NULL'},
...
def get_last_sync_log(unidad_negocio_pk: str) -> Optional[Dict]:
...
def get_stats_by_unidad_v2() -> List[Dict]:
    ...
    FROM vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime
    WHERE activo = 1
    GROUP BY unidad_negocio_pk, unidad_negocio_nombre, sistema_origen
