...
class CrearAsignacionRequest(BaseModel):
    ...
    unidad_negocio_id: str = Field(..., description="ID de la unidad de negocio")
...
class ActualizarAsignacionRequest(BaseModel):
    ...
    unidad_negocio_id: Optional[str] = Field(None, description="Nueva unidad de negocio")
...
async def validar_alcance_usuario(current_user: dict, unidad_negocio_id: str):
    ...
    if unidad_negocio_id not in empresas_permitidas:
        raise HTTPException(
            status_code=403,
...
async def validar_usuario_responsable(usuario_id: str, unidad_negocio_id: str):
    """
...
@router.get("/almacenes/{unidad_negocio_id}")
async def listar_almacenes(
    ...
    """
    # Validar alcance
    await validar_alcance_usuario(current_user, unidad_negocio_id)
    
    repo = get_config_asignaciones_repository(get_db())
    almacenes = await repo.listar_almacenes(unidad_negocio_id)
    
    return {"success": True, "data": almacenes}


@router.get("/almacenes/{unidad_negocio_id}/sync-info")
...
async def obtener_asignacion(
    ...
    # Validar alcance
    await validar_alcance_usuario(current_user, config["unidad_negocio_id"])
...
async def crear_asignacion(
    ...
    await validar_alcance_usuario(current_user, request.unidad_negocio_id)
    
    # 2. Validar unidad de negocio
    empresa = await get_db().empresas.find_one(
        {"id": request.unidad_negocio_id, "activa": True},
        {"_id": 0}
    )
    if not empresa:
    ...
    await validar_usuario_responsable(request.usuario_responsable_id, request.unidad_negocio_id)
    
    # 4. Validar almacén si se especifica
    if request.almacen_id:
        repo = get_config_asignaciones_repository(get_db())
        almacenes = await repo.listar_almacenes(request.unidad_negocio_id)
...
async def actualizar_asignacion(
    ...
    await validar_alcance_usuario(current_user, config_actual["unidad_negocio_id"])
    
    # Determinar valores finales
    nueva_unidad = request.unidad_negocio_id or config_actual["unidad_negocio_id"]
    ...
    if request.unidad_negocio_id and request.unidad_negocio_id != config_actual["unidad_negocio_id"]:
        await validar_alcance_usuario(current_user, request.unidad_negocio_id)
    ...
    cambio_clave = (
        (request.unidad_negocio_id and request.unidad_negocio_id != config_actual["unidad_negocio_id"]) or
    ...
    if cambio_clave:
        ...
        existente = await get_db().config_asignaciones.find_one({
            "unidad_negocio_id": nueva_unidad,
    ...
    # Auditoría
    cambios = {}
    if request.unidad_negocio_id:
        cambios["nueva_unidad"] = request.unidad_negocio_id
...
async def resolver_responsable_test(
    ...
    try:
        context = await resolve_unidad_context(current_user, unidad_negocio_id)
...
@router.post("/almacenes/sincronizar/{unidad_negocio_id}")
async def sincronizar_almacenes(
    ...
    empresa = await get_db().empresas.find_one({"id": unidad_negocio_id, "activa": True})
    if not empresa:
