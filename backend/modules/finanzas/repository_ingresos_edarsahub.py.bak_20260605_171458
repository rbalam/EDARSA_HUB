...
async def get_unidades_activas() -> List[Dict]:
    ...
    try:
        ...
        return [
            {
                'unidad_negocio_pk': str(u['unidad_negocio_pk']),
                'nombre': u['nombre'],
                'codigo': u['codigo']
            }
            for u in unidades
...
class RepositorioIngresosEDARSAHUB:
    ...
    async def get_cortes_caja(
        ...
        Args:
            unidad_negocio_pk: UUID de la unidad (opcional, None = todas permitidas)
    ...
    def _transform_corte(self, row: Dict) -> Dict:
        ...
        return {
            ...
            "unidad_negocio_pk": str(row.get('UnidadNegocioID') or ''),
    ...
    async def get_resumen_por_unidad(
        ...
        try:
            ...
            for row in rows:
                ...
                resumen.append({
                    'unidad_negocio_pk': str(row['UnidadNegocioID']),
