...
async def get_user_unidades_permitidas(current_user: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        cursor.execute(f"""
            SELECT CAST(id AS NVARCHAR(100)) as unidad_negocio_pk
        ...
        unidades = [str(row['unidad_negocio_pk']) for row in cursor.fetchall()]
        conn.close()
...
async def listar_cortes_caja(
    unidad_negocio_pk: Optional[str] = Query(None, description="UUID de la unidad de negocio"),
    sucursal_id: Optional[int] = Query(None, description="DEPRECATED: Usar unidad_negocio_pk"),
