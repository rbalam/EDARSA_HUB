...
DICIEMBRE 2026: FINANZAS-CXP-MPRO-COMBINE-01 - Migrado a subprocess para evitar encoding issues
DICIEMBRE 2026: FINANZAS-CXP-MPRO-CREDENTIALS-SECURITY-01:
    - Credenciales desde EDARSAHUB/server_registry.py (no MongoDB)
    - Subprocess recibe credenciales via stdin (no argv)
"""
...
# Usar subprocess helper para evitar problemas de encoding
from .sql_subprocess_helper import execute_sql_subprocess_secure

# ID del servidor MPRO
...
class FinanzasRepositoryMPRO:
    ...
    FUENTE DE CREDENCIALES: EDARSAHUB SQL via server_registry.py
    """
    
    def __init__(self, db):
        """
        Args:
            db: Instancia de MongoDB (solo para fallback legacy, preferir SQL)
        """
        self.db = db
    ...
    async def _get_credentials(self) -> Optional[Dict]:
        """
        Obtiene credenciales del servidor MPRO desde server_registry.py.
        
        FUENTE: EDARSAHUB SQL (config_origin = EDARSAHUB_SQL)
        FALLBACK: MongoDB solo si SQL falla (config_origin = MONGODB_LEGACY)
        """
        ...
        try:
            ...
            # Obtener servidor desde EDARSAHUB SQL (prioridad) con secretos sin enmascarar
            server = await get_server_by_id(
                server_id=MPRO_SERVER_ID,
                db=self.db,
                prefer_sql=True,
                allow_mongo_fallback=True,
                mask_secrets=False  # Necesitamos el password real
            )
            
            if not server:
                logging.error("[FinanzasRepoMPRO] Servidor MPRO no encontrado en registry")
                return None
            
            # Obtener credenciales descifradas
            credentials = get_decrypted_credentials(server)
            
            # Agregar config_origin para auditoría
            credentials['config_origin'] = server.get('config_origin', 'UNKNOWN')
            
            self._credentials_cache = credentials
            logging.info(f"[FinanzasRepoMPRO] Credenciales obtenidas desde {credentials['config_origin']}")
            
            return self._credentials_cache
            
        except Exception as e:
            logging.error(f"[FinanzasRepoMPRO] Error obteniendo credenciales: {type(e).__name__}")
            return None
    
    async def _execute_query(self, query: str, timeout: int = 30) -> List[Dict]:
        """
        Ejecuta una query en MPRO usando subprocess seguro.
        
        Las credenciales se pasan via stdin, NO por argumentos CLI.
        """
        ...
        try:
            result = await execute_sql_subprocess_secure(
            ...
            if result.get('success'):
                ...
                # No incluir detalles de credenciales en el error
                logging.error(f"[FinanzasRepoMPRO] Error subprocess: {result.get('error_type', 'UNKNOWN')}")
                return []
        except Exception as e:
            logging.error(f"[FinanzasRepoMPRO] Error ejecutando query: {type(e).__name__}")
            return []
    ...
    async def get_cuentas_por_pagar(
        ...
        # ABRIL 2026 CORRECCIÓN DEFINITIVA - MAPEO CAMPOS MPRO (según usuario):
        # TABLA: Cuenta_X_Pagar
        ...
        # - Cxp_Concepto = Comentario/Referencia
        # - Cxp_Folio = ID interno (para trazabilidad, NO mostrar como folio operativo)
        # - Cxp_Fecha = Fecha de entrada
        # - Cxp_Fecha_Vencimiento = Fecha de vencimiento
        # 
        # CLASIFICACIÓN MPRO por Grupo_Proveedor:
        ...
        query = f"""
            SELECT TOP {limit}
                ...
                c.Cxp_Documento as FolioEntrada,
                c.Cxp_Referencia as FolioFactura,
                ...
                p.Gp_Cve_Grupo_Proveedor as GrupoProveedor,
