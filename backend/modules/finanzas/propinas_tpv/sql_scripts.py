"""
EDARSA HUB - Script DDL para Módulo de Propinas TPV
===================================================
Fecha: 15 de Abril de 2026
Autor: Arquitecto EDARSA HUB
CAB Aprobado: ARQUITECTURA_PROPINAS_TPV_v3.md

IMPORTANTE:
- Ejecutar en SQL Server EDARSA HUB (edarsa_hub database)
- NO ejecutar en SoftRestaurant ni MPRO
- Este script es idempotente (usa IF NOT EXISTS)

TABLAS:
1. propinas_tpv_control - Registro principal de propinas
2. propinas_tpv_config - Configuración jerárquica
3. propinas_tpv_historial - Auditoría de cambios
"""

# ==============================================================================
# SCRIPT SQL PARA CREAR TABLAS EN EDARSA HUB
# ==============================================================================

SCRIPT_CREAR_PROPINAS_TPV_CONTROL = """
-- ============================================================================
-- TABLA: propinas_tpv_control
-- Propósito: Registro oficial de control y cuadre de propinas TPV por corte
-- Fuente de verdad: SQL Server EDARSA HUB
-- ============================================================================

IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'propinas_tpv_control')
BEGIN
    CREATE TABLE propinas_tpv_control (
        -- ============================================
        -- IDENTIFICADORES (Llave primaria + UUID)
        -- ============================================
    ...
    PRINT 'Tabla propinas_tpv_control creada exitosamente';
END
ELSE
BEGIN
    PRINT 'Tabla propinas_tpv_control ya existe';
END
"""

SCRIPT_CREAR_PROPINAS_TPV_CONFIG = """
-- ============================================================================
-- TABLA: propinas_tpv_config
-- Propósito: Configuración jerárquica de parámetros de propinas
-- Jerarquía: GLOBAL -> EMPRESA -> SUCURSAL
-- ============================================================================

IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'propinas_tpv_config')
BEGIN
    ...
    PRINT 'Tabla propinas_tpv_config creada exitosamente';
END
ELSE
BEGIN
    PRINT 'Tabla propinas_tpv_config ya existe';
END
"""

SCRIPT_CREAR_PROPINAS_TPV_HISTORIAL = """
-- ============================================================================
-- TABLA: propinas_tpv_historial
-- Propósito: Auditoría de todos los cambios realizados a propinas
-- ============================================================================

IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'propinas_tpv_historial')
BEGIN
    CREATE TABLE propinas_tpv_historial (
        ...
        CONSTRAINT FK_historial_propina 
            FOREIGN KEY (propina_id) 
            REFERENCES propinas_tpv_control(id)
    );

    -- Índices
    CREATE INDEX IX_historial_propina 
    ON propinas_tpv_historial (propina_id, fecha DESC);

    CREATE INDEX IX_historial_usuario 
    ON propinas_tpv_historial (usuario_id, fecha DESC);

    PRINT 'Tabla propinas_tpv_historial creada exitosamente';
END
ELSE
BEGIN
    PRINT 'Tabla propinas_tpv_historial ya existe';
END
"""

SCRIPT_INSERTAR_CONFIG_GLOBAL = """
-- ============================================================================
-- INSERT: Configuración GLOBAL por defecto
-- Solo si no existe ninguna configuración GLOBAL activa
-- ============================================================================

IF NOT EXISTS (SELECT 1 FROM propinas_tpv_config WHERE alcance_tipo = 'GLOBAL' AND activa = 1)
BEGIN
    INSERT INTO propinas_tpv_config (
        ...
        dias_para_cuadrar,
        soft_concepto_propinas,
        soft_conceptos_tarjeta,
        soft_concepto_efectivo,
        created_by,
        motivo_cambio
    ) VALUES (
        'GLOBAL',
        ...
        1,       -- 1 día hábil
        9,       -- Propinas pagadas
        '10,11,12',  -- VISA, MC, AMEX
        2,       -- Efectivo
        'sistema',
        'Configuración inicial del módulo Propinas TPV'
    );
    
    PRINT 'Configuración GLOBAL insertada';
END
ELSE
BEGIN
    PRINT 'Ya existe configuración GLOBAL activa';
END
"""

# Script completo para ejecutar todas las creaciones
SCRIPT_COMPLETO_DDL = f"""
-- ============================================================================
-- EDARSA HUB - Módulo Propinas TPV
-- Script DDL Completo
-- Fecha: 15/04/2026
-- ============================================================================

PRINT '=== Iniciando creación de tablas de Propinas TPV ===';
PRINT '';
