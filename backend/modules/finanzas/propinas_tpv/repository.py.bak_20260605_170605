...
DICIEMBRE 2026 - Migrado a subprocess para fix de encoding
...
from .schema_detector_subprocess import SoftRestaurantSchemaDetectorSubprocess
...
class PropinasTPVRepository:
    ...
    async def get_propinas_cortes_subprocess(
        self,
        server: Dict[str, Any],
        fecha_inicio: str,
        fecha_fin: str
        ...
        """
        DICIEMBRE 2026: Versión con subprocess para fix de encoding.
        
        Esta versión usa subprocess con locale UTF-8 forzado para evitar
        problemas de encoding con pytds en ambiente supervisor.
        ...
        try:
            # Paso 1: Detectar esquema usando subprocess
            logger.info(f"[Subprocess] Detectando esquema para {server['name']}...")
            schema = await SoftRestaurantSchemaDetectorSubprocess.detect_schema(server)
            result['schema'] = {
                'servidor': schema.get('servidor'),
                'compatible': schema.get('compatible'),
                'estrategia': schema.get('estrategia'),
                'tablas': schema.get('tablas_detectadas', []),
                'problemas': schema.get('problemas', [])
            }
            result['compatible'] = schema.get('compatible', False)
            
            if not schema.get('compatible'):
                result['error'] = f"Esquema no compatible: {schema.get('problemas', [])}"
                logger.warning(f"[Subprocess] {server['name']}: {result['error']}")
                return result
            
            # Paso 2: Construir y ejecutar query usando subprocess
            # Query para obtener propinas por corte
            estacion_col = schema.get('columna_estacion', 'estacion')
            ...
            SELECT 
                ...
                mc.folio AS folio_corte,
                mc.fecha AS fecha_corte,
                ch.{estacion_col} AS estacion_id,
                mc.idmovtocaja AS corte_id,
                ISNULL(SUM(ch.propinatarjeta), 0) AS propinas_tpv,
                ISNULL(SUM(ch.propina), 0) AS propinas_totales,
                ISNULL(SUM(CASE WHEN ch.tipodepago IN (2,3,4) THEN ch.total ELSE 0 END), 0) AS ventas_tarjeta,
                ISNULL(SUM(CASE WHEN ch.tipodepago = 1 THEN ch.total ELSE 0 END), 0) AS ventas_efectivo,
                ISNULL(SUM(ch.total), 0) AS ventas_totales,
                COUNT(ch.folio) AS total_cheques,
                mc.saldo AS saldo_corte,
                'CHEQUES_PROPINATARJETA' AS estrategia_usada
            FROM cheques ch
            INNER JOIN turnos t ON ch.idturno = t.idturno
            LEFT JOIN movtoscaja mc ON t.idturno = mc.idturno AND mc.idtipomovtocaja = 3
            WHERE ch.fecha BETWEEN '{fecha_inicio}' AND '{fecha_fin} 23:59:59'
              AND (ch.cancelado IS NULL OR ch.cancelado = 0)
              AND ch.pagado = 1
            GROUP BY t.idturno, mc.folio, mc.fecha, ch.{estacion_col}, mc.idmovtocaja, mc.saldo
            HAVING ISNULL(SUM(ch.propinatarjeta), 0) > 0
            ORDER BY mc.fecha DESC
            """
            
            result['query_usada'] = query[:500] + '...' if len(query) > 500 else query
            
            logger.info(f"[Subprocess] Ejecutando query para {server['name']}...")
            
            query_result = await execute_sql_subprocess(
                host=server['host'],
                port=server['port'],
            ...
            if not query_result.get('success'):
                result['error'] = f"Error en query: {query_result.get('error')}"
                logger.error(f"[Subprocess] {server['name']}: {result['error']}")
                return result
            
            # Paso 3: Procesar resultados
            cortes = []
            estrategia_detectada = 'CHEQUES_PROPINATARJETA'
            
            for row in query_result.get('data', []):
                fecha_val = row.get('fecha_corte')
                fecha_iso = fecha_val if isinstance(fecha_val, str) else (fecha_val.isoformat() if fecha_val else None)
                
                cortes.append({
                    'folio_corte': str(row.get('folio_corte', '') or 'SIN_CORTE'),
                    'fecha_corte': fecha_iso,
                    'estacion_id': str(row.get('estacion_id', 'N/A')),
                    'turno_id': str(row.get('turno_id', 'N/A')),
                    'corte_id': str(row.get('corte_id', '')),
            ...
            result['cortes'] = cortes
            result['estrategia_usada'] = estrategia_detectada
            logger.info(f"[Subprocess] {server['name']}: {len(cortes)} cortes encontrados")
            
        except Exception as e:
            result['error'] = str(e)
            logger.error(f"[Subprocess] Error para {server['name']}: {e}")
