...
async def obtener_estado_detector(
    ...
    ultima = db.scheduler_job_logs.find_one(
...
async def listar_tareas_operativas(
    ...
    tareas = list(db.tareas_operativas_compras.find(
        filtro,
        {"_id": 0}
...
async def obtener_tarea_operativa(
    ...
    tarea = db.tareas_operativas_compras.find_one(
        {"id": tarea_id},
        {"_id": 0}
    )
...
async def completar_tarea_operativa(
    ...
    tarea = db.tareas_operativas_compras.find_one({"id": tarea_id})
    if not tarea:
        raise HTTPException(status_code=404, detail="Tarea no encontrada")
    ...
    db.tareas_operativas_compras.update_one(
        {"id": tarea_id},
        {
            "$set": {
                "estado": "COMPLETADA",
                "fecha_completado": now,
                "completado_por": payload.get("user_id"),
...
async def obtener_bitacora_detector(
    ...
    bitacora = list(db.auditoria_compras_bitacora.find(
        {},
        {"_id": 0}
...
async def listar_pedidos_procesados(
    ...
    pedidos = list(db.pedidos_procesados_automatizacion.find(
        filtro,
        {"_id": 0}
    ).sort("fecha_procesado", -1).limit(limite))
