...
async def descargar_excel_workflow(
    ...
    try:
        ...
        # SQL-FIRST P2B: registrar en dbo.Operativo_DocumentosGenerados
        try:
            from modules.compras.sync_service import get_edarsahub_connection
            import uuid
            conn = get_edarsahub_connection()
            cur = conn.cursor(as_dict=True)
            cur.execute("SELECT ISNULL(MAX(ID),0)+1 AS NextID FROM dbo.Operativo_DocumentosGenerados WITH (UPDLOCK,HOLDLOCK)")
            row = cur.fetchone() or {}
            next_id = int(row.get("NextID") or 1)
            cur.execute("""
                INSERT INTO dbo.Operativo_DocumentosGenerados (
                    ID, DocumentoID, WorkflowID, TipoDocumento, NombreArchivo,
                    Formato, UsuarioGeneradorID, Estado, FechaGeneracion, MetadatosJSON
                )
                VALUES (%s,%s,%s,%s,%s,%s,%s,'GENERADO',GETUTCDATE(),%s)
            """, (
                next_id,
                str(uuid.uuid4()),
                workflow_id,
