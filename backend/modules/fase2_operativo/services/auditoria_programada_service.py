...
Usa PyMongo sync para compatibilidad con el módulo.
"""
...
class AuditoriaProgramadaService:
    ...
    def crear(self, data: AuditoriaProgramadaCreate) -> Dict:
        ...
        return self.repo.create(auditoria.model_dump())
    
    def obtener(self, auditoria_id: str) -> Dict:
        """Obtiene una auditoría por ID."""
        auditoria = self.repo.get_by_id(auditoria_id)
        if not auditoria:
    ...
    def listar(
        ...
        if sucursal_id:
            return self.repo.get_by_sucursal(sucursal_id)
        elif solo_activas:
            return self.repo.get_activas()
        else:
            return self.repo.get_all()
    
    def actualizar(self, auditoria_id: str, data: AuditoriaProgramadaUpdate) -> Dict:
        ...
        if any(k in update_data for k in ['frecuencia', 'dia_semana', 'dia_mes', 'hora_ejecucion', 'timezone']):
            ...
            if frecuencia != FrecuenciaAuditoria.MANUAL.value:
                ...
                update_data['proxima_ejecucion'] = proxima.isoformat() if proxima else None
        
        update_data['updated_at'] = datetime.now(timezone.utc).isoformat()
        return self.repo.update(auditoria_id, update_data)
    
    def eliminar(self, auditoria_id: str) -> bool:
        """Elimina una auditoría programada."""
        self.obtener(auditoria_id)
        return self.repo.delete(auditoria_id)
    
    def activar(self, auditoria_id: str) -> Dict:
        """Activa una auditoría."""
        auditoria = self.obtener(auditoria_id)
        ...
        return self.obtener(auditoria_id)
    
    def desactivar(self, auditoria_id: str) -> Dict:
        """Desactiva una auditoría."""
        self.obtener(auditoria_id)
        self.repo.desactivar(auditoria_id)
        return self.obtener(auditoria_id)
    
    def ejecutar_manual(self, auditoria_id: str, usuario_id: str) -> Dict:
    ...
    def obtener_kpis(self) -> Dict:
        """Obtiene KPIs de auditorías programadas."""
        ahora = datetime.now(timezone.utc)
        
        conteo = self.repo.contar_por_estado()
        
        fin_hoy = ahora.replace(hour=23, minute=59, second=59)
        pendientes_hoy = self.repo.get_pendientes_ejecucion(fin_hoy)
        
        logs_mes = self.repo.contar_logs_mes(ahora.year, ahora.month)
        
        proximas = self.repo.get_proximas_24h()
    ...
    def obtener_calendario(self, anio: int, mes: int) -> Dict:
        """Obtiene vista calendario."""
        auditorias = self.repo.get_calendario(anio, mes)
    ...
    def obtener_historial(
        ...
        return self.repo.get_logs(auditoria_id=auditoria_id, estado=estado, desde=desde, limit=limit)
