...
class TestCatalogosDisponibles:
    ...
    def test_catalogos_have_required_fields(self, admin_token):
        ...
        for cat in catalogos:
            assert "id" in cat, f"Missing 'id' in catalog"
            assert "nombre" in cat, f"Missing 'nombre' in catalog"
            assert "modulo" in cat, f"Missing 'modulo' in catalog"
            assert "tabla" in cat, f"Missing 'tabla' in catalog"
        
        print(f"✅ All catalogs have required fields (id, nombre, modulo, tabla)")
...
class TestMisTareas:
    ...
    def test_get_mis_tareas_admin(self, admin_token):
        ...
        print(f"✅ GET /api/sistema/mis-tareas returns correct structure")
...
class TestMisPermisosCatalogos:
    ...
    def test_admin_has_all_permissions(self, admin_token):
        ...
        assert data["puede_solicitar"] == True
        assert data["puede_aprobar"] == True
    ...
    def test_assign_permissions_requires_supervisor_or_admin(self, admin_token, supervisor_token):
        ...
        if len(usuarios) > 0:
            ...
            if test_user:
                ...
                print(f"✅ Admin can assign permissions to users")
                ...
                print(f"✅ Supervisor can assign permissions to users")
        else:
...
class TestUsuariosAsignables:
    ...
    def test_get_usuarios_asignables_supervisor(self, supervisor_token):
        ...
        assert response.status_code == 200, f"Failed: {response.text}"
        print(f"✅ Supervisor can access usuarios-asignables endpoint")
...
class TestAprobacionConFirma:
    ...
    def test_supervisor_can_approve(self, supervisor_token):
        ...
        if create_response.status_code == 200:
            ...
            print(f"✅ Supervisor can approve solicitudes with password signature")
        else:
            print(f"⚠️ Supervisor cannot create solicitudes (expected if no permissions assigned)")
