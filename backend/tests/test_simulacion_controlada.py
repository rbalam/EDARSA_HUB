...
parser.add_argument('--persistir-lote-mpro-2', action='store_true',
                    help='Fase 1B.4A-MPRO: Persistir lote de máximo 2 registros MPRO con confirmación por registro')
...
elif args.persistir_lote_mpro_2:
    MODO = 'PERSISTIR_LOTE_MPRO_2'
else:
    MODO = 'DRY_RUN'
...
# =============================================================================
# MODO PERSISTIR LOTE MPRO 2 (FASE 1B.4A-MPRO)
# =============================================================================
# Límites estrictos HARDCODEADOS:
#   - SOLO sistema MPRO (SOFT no habilitado)
#   - MÁXIMO 2 registros
...
LIMITE_LOTE_FASE_1B4A_MPRO = 2  # Máximo 2 registros MPRO - NO MODIFICAR


def generar_identificador_lote_mpro2() -> str:
    """
    Genera identificador único para lote MPRO Fase 1B.4A.
    Formato: CAB003_FASE1B4A_MPRO2_YYYYMMDD_HHMMSS
    """
    return f"CAB003_FASE1B4A_MPRO2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"


def mostrar_payload_registro_mpro2(inv: dict, numero: int, total: int, identificador: str, hash_verif: str, es_duplicado: bool):
    """Muestra el payload completo de un registro MPRO para Fase 1B.4A."""
    print("\n" + "┌" + "─" * 78 + "┐")
    print(f"│ REGISTRO {numero} de {total} (MPRO)".ljust(79) + "│")
    print("├" + "─" * 78 + "┤")
    print(f"│  {'Campo':<28} │ {'Valor':<45} │")
    print("├" + "─" * 30 + "┼" + "─" * 47 + "┤")
    print(f"│  {'sistema_origen':<28} │ {inv['sistema_origen']:<45} │")
    print(f"│  {'server_id':<28} │ {inv['server_id']:<45} │")
    print(f"│  {'sucursal_id':<28} │ {(inv['sucursal_id'] or '(vacío)'):<45} │")
    sucursal_nombre = inv.get('sucursal_nombre', 'N/A')[:40]
    print(f"│  {'sucursal_nombre':<28} │ {sucursal_nombre:<45} │")
    print(f"│  {'almacen_id':<28} │ {inv['almacen_id']:<45} │")
    almacen_nombre = inv.get('almacen_nombre', 'N/A')[:40]
    print(f"│  {'almacen_nombre':<28} │ {almacen_nombre:<45} │")
    comentario = (inv.get('comentario') or 'NULL')[:40]
    print(f"│  {'comentario':<28} │ {comentario:<45} │")
    print(f"│  {'folio_inventario':<28} │ {inv['folio_inventario']:<45} │")
    print(f"│  {'fecha_inventario':<28} │ {str(inv['fecha_inventario']):<45} │")
    estado_origen = inv.get('estado_inventario_origen') or 'NULL'
    print(f"│  {'estado_inventario_origen':<28} │ {estado_origen:<45} │")
    print(f"│  {'hash_verificacion':<28} │ {hash_verif[:40]+'...':<45} │")
    print(f"│  {'estado':<28} │ {'EN_PROCESO':<45} │")
    print(f"│  {'created_by':<28} │ {identificador:<45} │")
    print("├" + "─" * 30 + "┼" + "─" * 47 + "┤")
...
async def ejecutar_persistir_lote_mpro_2():
    """
    Fase 1B.4A-MPRO: Persistir lote de máximo 2 registros MPRO.
    ...
    """
    print("\n" + "=" * 80)
    print("🔒 FASE 1B.4A-MPRO - PERSISTIR LOTE MPRO (MÁXIMO 2 REGISTROS)")
    print("=" * 80)
    print("""
    Este proceso insertará hasta 2 registros MPRO con:
    ...
    # PASO 0: CONFIRMACIÓN DE INICIO
    # =========================================================================
    inicio = solicitar_confirmacion("INICIAR proceso lote MPRO (máximo 2 registros)")
    if inicio == 'salir':
        print("\n🛑 Proceso abortado por usuario")
        return
    if inicio == 'n':
    ...
    print("\n" + "─" * 80)
    print(f"CANDIDATOS MPRO ELEGIBLES (accion_sugerida = 'PROCESAR'): {len(candidatos_mpro)}")
    print(f"REGISTROS EN ESTE LOTE: {min(len(candidatos_mpro), LIMITE_LOTE_FASE_1B4A_MPRO)} (máximo {LIMITE_LOTE_FASE_1B4A_MPRO})")
    print("─" * 80)
    ...
    # Tomar máximo 2 registros (HARDCODEADO)
    candidatos_lote = candidatos_mpro[:LIMITE_LOTE_FASE_1B4A_MPRO]
    
    continuar = solicitar_confirmacion(f"CONTINUAR con selección de {len(candidatos_lote)} registro(s) MPRO")
    if continuar == 'salir':
        print("\n🛑 Proceso abortado por usuario")
        return
    if continuar == 'n':
    ...
    # PASO 3: GENERAR IDENTIFICADOR ÚNICO DE LOTE
    # =========================================================================
    identificador_lote = generar_identificador_lote_mpro2()
    ...
    for i, candidato in enumerate(candidatos_validados, 1):
        ...
        # Mostrar payload completo
        mostrar_payload_registro_mpro2(inv, i, len(candidatos_validados), identificador_lote, hash_verif, es_duplicado)
        ...
        # Solicitar confirmación OBLIGATORIA para este registro específico
        confirmacion = solicitar_confirmacion(f"Registro {i}/{len(candidatos_validados)} → ¿Insertar este registro MPRO?")
            ...
            for j in range(i, len(candidatos_validados) + 1):
                if j > i or (j == i and not candidatos_validados[j-1]['es_duplicado']):
                    registros_fallidos.append({
                        'numero': j,
                        'folio': candidatos_validados[j-1]['inventario']['folio_inventario'],
                        'sucursal': candidatos_validados[j-1]['inventario']['sucursal_id'],
                        'error': 'Abortado por usuario'
                    })
            break
    ...
    for r in registros_insertados:
        print(f"      • Registro {r['numero']}: Folio {r['folio']}, Sucursal {r['sucursal']}, Almacén {r['almacen']}")
        print(f"        Comentario: {r['comentario']}")
        print(f"        procesado_id: {r['procesado_id']}")
    
    print(f"""
    OMITIDOS: {len(registros_omitidos)}""")
    
    for r in registros_omitidos:
        print(f"      • Registro {r['numero']}: Folio {r['folio']}, Sucursal {r['sucursal']} → {r['razon']}")
    
    print(f"""
    NO INSERTADOS (error/abortado): {len(registros_fallidos)}""")
    
    for r in registros_fallidos:
        print(f"      • Registro {r['numero']}: Folio {r['folio']}, Sucursal {r['sucursal']} → {r['error']}")
...
async def main():
    ...
    elif MODO == 'PERSISTIR_LOTE_MPRO_2':
        await ejecutar_persistir_lote_mpro_2()
    else:
