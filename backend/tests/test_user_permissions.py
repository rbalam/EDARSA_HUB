...
TEST_USER_EMAIL = "test@inventario.com"
...
class TestLoginWithCredentials:
    ...
    def test_test_user_login_success(self):
        ...
        if response.status_code != 200:
            print(f"Test user not found or login failed - may need to be created first")
...
class TestPermissionsRestrictedUserAccess:
    ...
    def test_restricted_user_sees_only_assigned_servers(self, test_user_headers, admin_headers):
        ...
        if allowed_servers:
            ...
            print(f"✓ Permission filtering working - user only sees assigned servers")
        else:
