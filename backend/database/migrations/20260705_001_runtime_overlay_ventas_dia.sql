-- Runtime KPI overlay: historico cerrado filtrado + snapshot operativo vigente.
-- No modifica dbo.Comercial_KPIs_Diarios_v2.
-- No expone columnas activo/es_demo; el filtro vive dentro de la vista.

CREATE OR ALTER VIEW dbo.vw_Comercial_KPIs_Diarios_v2_Runtime
AS
...
base_overlay AS (
    SELECT
        ...
        CASE WHEN s.unidad_negocio_id IS NOT NULL THEN CAST(0 AS decimal(18,2)) ELSE k.propinas_total END AS propinas_total,
...
CREATE OR ALTER VIEW dbo.vw_Comercial_KPIs_Diarios_v2_Canonica
AS
SELECT
    k.unidad_negocio_pk,
    k.unidad_id,
    k.unidad_codigo,
    k.unidad_nombre_catalogo AS unidad_nombre,
    k.unidad_negocio_id AS unidad_codigo_legacy,
    k.unidad_negocio_nombre AS unidad_nombre_legacy,
    k.fecha_operacion,
    k.ventas_sin_propina AS VentaNeta,
    k.tickets_total AS Cheques,
    k.pax_total AS PAX,
    k.ticket_promedio AS TicketPromedio,
    k.pax_promedio AS PaxPromedio
