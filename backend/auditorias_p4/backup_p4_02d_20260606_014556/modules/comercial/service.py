...
# Tablero Ejecutivo lee exclusivamente de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime.
...
def _obtener_codigo_canonico_mpro(server_id: str, sucursal_id: str, sucursal_nombre: str) -> tuple:
    ...
    # =========================================================================
    fallback_map = {
        '0021': (UnidadesService.resolver_codigo('130QRO') or '130QRO', '130° QUERETARO'),
        '0023': (UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN', UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN'),
        UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN': (UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN', UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN'),
        '130_QRO': (UnidadesService.resolver_codigo('130QRO') or '130QRO', '130° QUERETARO'),
    }
...
def _mapear_codigo_a_unidad_negocio_pk(codigo_empresa: str) -> str:
    """
    ...
    REGLA DE NEGOCIO:
    - La identidad principal es unidad_negocio_pk CANÓNICO (130MID, 130QRO, etc.)
    ...
    - Los datos en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime deben usar códigos canónicos
    ...
    normalizacion_a_canonico = {
        ...
        '130-MID': UnidadesService.resolver_codigo('130MID') or '130MID',
        '130MER': UnidadesService.resolver_codigo('130MID') or '130MID',
        '130 MERIDA': UnidadesService.resolver_codigo('130MID') or '130MID',
        '130 MÉRIDA': UnidadesService.resolver_codigo('130MID') or '130MID',
        '130° MERIDA': UnidadesService.resolver_codigo('130MID') or '130MID',
        '130° MÉRIDA': UnidadesService.resolver_codigo('130MID') or '130MID',
        'MERIDA': UnidadesService.resolver_codigo('130MID') or '130MID',
        'MÉRIDA': UnidadesService.resolver_codigo('130MID') or '130MID',
        # Querétaro - todas las variantes resuelven a 130QRO
        '130-QRO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        '130 QRO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        '130 QUERETARO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        '130 QUERÉTARO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        '130° QUERETARO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        '130° QUERÉTARO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        'QUERETARO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        'QUERÉTARO': UnidadesService.resolver_codigo('130QRO') or '130QRO',
        # La Estelar
...
def _query_edarsahub_tablero(query: str) -> List[Dict]:
    ...
    Fuente: vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime y Comercial_Ventas_Dia_Abiertas_v2.
...
def _get_ultimo_dia_con_datos_edarsahub(server_id: str, mes: int, anio: int) -> Optional[int]:
    """
    Obtiene el último día con ventas registradas en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime.
    Retorna el día (1-31) o None si no hay datos.
    """
    query = f"""
    SELECT MAX(dia) as ultimo_dia
    FROM vw_vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime_Runtime
...
def _get_kpis_periodo_edarsahub(
    ...
    unidad_negocio_pk: str = None  # FIX: Agregar unidad_negocio_pk como parámetro opcional
) -> Dict:
    """
    Obtiene KPIs agregados de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime para un período.
    
    FIX CIRCUIT BREAKER HUB (17-May-2026):
    - Si se proporciona unidad_negocio_pk, filtrar por unidad_negocio_pk + sucursal_id
    - Si no, usar server_id + sucursal_id (comportamiento legacy)
    
    RAZÓN: Para MPRO, los datos en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime tienen un server_id
    diferente al que está configurado en Servidores_Conexiones. Usar unidad_negocio_pk
    es más confiable.
...
def _get_ventas_abiertas_edarsahub(server_id: str, sucursal_id: str = 'DEFAULT', unidad_negocio_pk: str = None) -> Dict:
    """
    ...
    if unidad_negocio_pk:
        ...
        logging.info(
            f"[TABLERO-EDARSAHUB] _get_ventas_abiertas: {unidad_negocio_pk} FechaOperacion={fecha_operacion} "
    ...
    # =================================================================
    # FIX P0 (15-May-2026): Buscar por unidad_negocio_pk en lugar de server_id
    ...
    if unidad_negocio_pk:
        # Priorizar búsqueda por unidad_negocio_pk (más confiable)
    ...
    if (not result or len(result) == 0) and unidad_negocio_pk:
        logging.warning(
            f"[TABLERO-EDARSAHUB] Sin datos para fecha exacta. Buscando último snapshot para {unidad_negocio_pk}..."
        )
        ...
        if result and len(result) > 0:
            is_stale = True
            logging.warning(
                f"[TABLERO-EDARSAHUB] Usando snapshot STALE para {unidad_negocio_pk}: "
...
def _obtener_kpis_tablero_desde_edarsahub(
    ...
    - NO consulta MongoDB
    - Lee de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
    ...
    Args:
        ...
        unidad_negocio_pk: ID de unidad (130MID, CIENFUEGOS, etc.)
    ...
    """
    logging.info(f"[TABLERO-EDARSAHUB] {nombre_unidad or unidad_negocio_pk}: Consultando KPIs desde EDARSAHUB, dias_transcurridos={dias_transcurridos}")
    ...
    # FIX: Pasar unidad_negocio_pk para filtro más confiable (especialmente para MPRO)
    kpis_actual = _get_kpis_periodo_edarsahub(server_id, fecha_ini, fecha_fin_real, sucursal_id, unidad_negocio_pk=unidad_negocio_pk)
    ...
    kpis_mes_ant = _get_kpis_periodo_edarsahub(server_id, fecha_ini_ant, fecha_fin_ant, sucursal_id, unidad_negocio_pk=unidad_negocio_pk)
    ...
    kpis_anio_ant = _get_kpis_periodo_edarsahub(server_id, fecha_ini_anio_ant, fecha_fin_anio_ant, sucursal_id, unidad_negocio_pk=unidad_negocio_pk)
    ...
    return {
        ...
        "fuente": "EDARSAHUB",
        "_meta": {
            "tabla": "vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime",
...
def build_unit_response(
    ...
    response = {
        ...
        "unidad_negocio_pk": unidad_negocio_codigo or server_id,  # Código canónico, no server_id
...
# - Fuente: vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
...
def get_kpis_softrestaurant(server, fecha_ini, fecha_fin, fecha_ini_ant, fecha_fin_ant, fecha_ini_año_ant, fecha_fin_año_ant, dias_transcurridos, dias_mes, solo_ventas_dia=False):
    ...
    - Lee EXCLUSIVAMENTE de EDARSAHUB (vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime)
    ...
    # =========================================================================
    # FASE 5B: Usar EmpresaResolver para obtener mapeo de unidad_negocio_pk
    # El mapeo traduce códigos canónicos (130MID) a formatos legacy (130-MER)
    # usados en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
    # =========================================================================
    unidad_negocio_pk = _mapear_codigo_a_unidad_negocio_pk(unidad_negocio_codigo)
    
    logging.info(f"[TABLERO-EDARSAHUB] {nombre}: Iniciando consulta - unidad={unidad_negocio_pk}, sucursal={sucursal_id}")
    ...
    if solo_ventas_dia:
        ...
        ventas_abiertas = _get_ventas_abiertas_edarsahub(server_id, sucursal_id, unidad_negocio_pk=unidad_negocio_codigo)
    ...
    # ============================================================================
    # MODO KPIs HISTÓRICOS/ACUMULADOS: Leer de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
    # ============================================================================
...
def get_kpis_mpro(server, fecha_ini, fecha_fin, fecha_ini_ant, fecha_fin_ant, fecha_ini_año_ant, fecha_fin_año_ant, dias_transcurridos, dias_mes):
    ...
    for sucursal_id, sucursal_config in sucursales.items():
        unidad_negocio_pk = sucursal_config.get('unidad_negocio_pk', '')
...
def get_kpis_mpro_por_sucursal(server, fecha_ini, fecha_fin, fecha_ini_ant, fecha_fin_ant, fecha_ini_año_ant, fecha_fin_año_ant, dias_transcurridos, dias_mes, solo_ventas_dia=False):
    ...
    if solo_ventas_dia:
        ...
        for suc in sucursales_mpro:
            ...
            datos_edarsahub = _get_ventas_abiertas_edarsahub(
                ...
                unidad_negocio_pk=codigo_canonico
            )
    ...
    # - Usar la misma fuente que SoftRestaurant: vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
    # 
    ...
    for suc in sucursales_mpro:
        ...
        # Mapear código a unidad_negocio_pk usado en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
        unidad_negocio_pk = _mapear_codigo_a_unidad_negocio_pk(codigo_canonico)
...
# EDARSAHUB SÍ tiene datos en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime.
# 
...
#
# SOLUCIÓN: Usar la MISMA fuente que Tablero Ejecutivo (vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime)
...
def get_dashboard_kpis_from_edarsahub(
    ...
    """
    DASHBOARD COMERCIAL - KPIs desde EDARSAHUB (vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime)
    ======================================================================
    
    Usa la MISMA fuente que el Tablero Ejecutivo para garantizar consistencia.
    
    FUENTE: vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime en EDARSAHUB
    ...
    return {
        ...
        'source_table': 'vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime',
...
def _get_kpis_periodo_edarsahub_flexible(
    ...
    # =========================================================================
    # Este fallback maneja el caso donde los datos MPRO en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
...
def get_last_valid_snapshot_edarsahub(server_id: str) -> Dict:
    ...
    Busca en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime el registro más reciente para el servidor,
