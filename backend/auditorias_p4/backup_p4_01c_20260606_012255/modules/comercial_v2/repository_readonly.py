...
FUENTES ÚNICAS:
- vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime
- vw_vw_vw_Comercial_KPIs_Mensuales_v2_Runtime_Runtime_Runtime
...
def get_kpis_diarios(
    ...
    """
    Obtiene KPIs diarios desde vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime.
...
def get_kpis_por_unidad(
    ...
    - Se detectaron datos DUPLICADOS en vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime para Mérida
    ...
    if unidades_permitidas:
        ...
        for uid in unidades_permitidas:
            ...
            if uid == UnidadesService.resolver_codigo('130MID') or '130MID':
                expanded_ids.extend(['130-MER', '130-MID'])
            elif uid == UnidadesService.resolver_codigo('130QRO') or '130QRO':
                expanded_ids.extend(['130-QRO'])
            elif uid == UnidadesService.resolver_codigo('ESTELAR') or 'ESTELAR':
                expanded_ids.extend(['LA-ESTELAR'])
        ids_quoted = ','.join([f"'{u}'" for u in set(expanded_ids)])
        base_where.append(f"unidad_negocio_pk IN ({ids_quoted})")
    ...
    WITH datos_normalizados AS (
        SELECT 
            ...
            CASE 
                WHEN UPPER(unidad_negocio_nombre) LIKE '%MERIDA%' 
                  OR UPPER(unidad_negocio_nombre) LIKE '%MÉRIDA%' THEN UnidadesService.resolver_codigo('130MID') or '130MID'
                WHEN UPPER(unidad_negocio_nombre) LIKE '%QUERETARO%' 
                  OR UPPER(unidad_negocio_nombre) LIKE '%QUERÉTARO%' THEN UnidadesService.resolver_codigo('130QRO') or '130QRO'
                WHEN UPPER(unidad_negocio_nombre) LIKE '%CIENFUEGOS%' THEN UnidadesService.resolver_codigo('CIENFUEGOS') or 'CIENFUEGOS'
                WHEN UPPER(unidad_negocio_nombre) LIKE '%ESTELAR%' THEN UnidadesService.resolver_codigo('ESTELAR') or 'ESTELAR'
                WHEN UPPER(unidad_negocio_nombre) LIKE '%ORIGEN%' THEN UnidadesService.resolver_codigo('ORIGEN') or 'ORIGEN'
                ELSE unidad_negocio_pk
    ...
    SELECT 
        id_normalizado as unidad_negocio_pk,
...
def get_kpis_mensuales(
    ...
    """
    Obtiene KPIs mensuales desde vw_vw_vw_Comercial_KPIs_Mensuales_v2_Runtime_Runtime_Runtime.
    """
    where_clauses = [
        f"anio = {anio}",
    ...
    FROM vw_vw_vw_vw_Comercial_KPIs_Mensuales_v2_Runtime_Runtime_Runtime_Runtime
    WHERE {' AND '.join(where_clauses)}
...
def get_ventas_dia_abiertas(
    ...
    WITH RankedData AS (
        SELECT 
            ...
            sync_run_id,
            fecha_ultima_actualizacion,
            ROW_NUMBER() OVER (PARTITION BY unidad_negocio_pk ORDER BY snapshot_timestamp DESC) as rn
    ...
    try:
        ...
        if stats:
            return {
                "status": "ok",
                "source": "EDARSAHUB_V2",
                "table": "vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime",
...
def get_comparativos_diarios(
    ...
    - Día anterior (de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime)
    - Mismo día año anterior (de vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime)
