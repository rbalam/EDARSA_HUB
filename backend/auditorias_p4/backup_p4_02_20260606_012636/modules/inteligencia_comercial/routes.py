...
Fuentes de datos:
- KPIs Dashboard: vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime (pre-calculada)
...
async def get_ventas_familia(
    ...
    try:
        ...
        kpi_sql = f"SELECT SUM(ventas_total) AS total FROM vw_vw_vw_vw_Comercial_KPIs_Diarios_v2_Runtime_Runtime_Runtime_Runtime WHERE {' AND '.join(where_kpi)}"
        kpis = execute_query(kpi_sql)
        total_ventas = float(kpis[0]["total"] or 0) if kpis else 0
