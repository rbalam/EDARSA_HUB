...
#
# FASE 4.4: Aplicación de SourceQueryResult
# - Distingue consulta exitosa de error de conexión
...
def get_configuracion_activa(
    ...
    try:
        ...
        # FASE 4.4: Clasificar el error para mejor diagnóstico
        error_status = classify_sql_error(e)
        logger.error(f"Error leyendo configuración: {e} [Status: {error_status.value}]")
        return []
...
def get_folios_ya_procesados(
    ...
    try:
        ...
        # FASE 4.4: Clasificar el error para mejor diagnóstico
        error_status = classify_sql_error(e)
        logger.error(f"Error leyendo folios procesados: {e} [Status: {error_status.value}]")
        return []
...
def leer_configuracion_con_estado(
    execute_sql_func,
    host: str,
    port: int,
    database: str,
    ...
    """
    FASE 4.4: Lee configuración de automatización con envelope de estado.
    
    Distingue:
    - SUCCESS_WITH_DATA: Configuración encontrada
    - SUCCESS_EMPTY: No hay configuración (resultado real)
    - SOURCE_UNREACHABLE: Error de conexión (NUNCA se reporta como vacío)
    """
    import time
    ...
    query = """
        SELECT 
            id, server_id, sucursal_id, almacen_id,
            habilitado, intervalo_minutos,
            ultimo_folio_conocido,
...
def leer_folios_procesados_con_estado(
    execute_sql_func,
    host: str,
    port: int,
    database: str,
    ...
    """
    FASE 4.4: Lee folios procesados con envelope de estado.
    """
    import time
    start_time = time.time()
    
    query = f"""
        SELECT folio_inventario, fecha_proceso, estado
        FROM automatizacion_inventarios_folios_procesados
