...
"""
    ...
    logging.info(f"[TABLERO] SoftRestaurant {server['name']}: Iniciando consulta SQL - Host={server['host']}:{server['port']}, DB={server['database']}")
    try:
        test_query = "SELECT 1 as test"
        test_result = execute_sql_query(server['host'], server['port'], server['database'], 
                                        server['username'], server['password'], test_query)
        if not test_result:
            logging.warning(f"[TABLERO] SoftRestaurant {server['name']}: Conexión SQL falló - retornando None para usar caché")
            return None
...
"""
    ...
    try:
        result = execute_sql_query(server['host'], server['port'], server['database'], 
                                   server['username'], server['password'], query)
        logging.info(f"MPRO {server['name']}: Query retornó {len(result) if result else 0} sucursales")
        if not result:
            logging.warning(f"MPRO {server['name']}: No se encontraron sucursales con ventas")
            return []
    except Exception as e:
        # FASE 4.4: Registrar como SOURCE_UNREACHABLE en lugar de retornar []
        # El llamador debe manejar esto adecuadamente
        error_status = classify_sql_error(e)
        logging.warning(
            f"Error consultando MPRO por sucursal {server['name']}: {e} "
            f"[Status: {error_status.value}]"
        )
        # Por compatibilidad, aún retornamos [] pero con log claro
        # TODO: Refactorizar para retornar SourceQueryResult
        return []
