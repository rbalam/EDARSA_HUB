...
# ============================================================================
# BLINDAJE RBAC - VALIDACIÓN DE ACCESO A SERVIDORES (FASE COMERCIAL)
# ============================================================================

async def validate_server_access_rbac(current_user: Dict, server_id: str) -> bool:
    """
    Validación unificada de acceso a servidor según RBAC.
    REGLA: Un usuario solo puede acceder a servidores asociados a sus empresas_permitidas.
    ...
    """
    # FASE 3: Validar acceso por empresa primero (modelo nuevo)
    ...
    if empresas_permitidas:
        ...
        if server_id in servers_permitidos:
            return True
        # No tiene acceso por RBAC nuevo
        logging.warning(f"[RBAC] Usuario {current_user.get('email')} sin acceso a servidor {server_id}")
        raise HTTPException(status_code=403, detail="No tiene acceso a este servidor")
    
    # Fallback legacy: Usar allowed_servers si no tiene empresas asignadas
    if user_has_server_access(current_user, server_id):
        return True
...
async def comercial_ticket_perfecto(
    ...
    if not user_has_server_access(current_user, server_id):
        # FASE 3: Intentar validación RBAC primero
        await validate_server_access_rbac(current_user, server_id)
