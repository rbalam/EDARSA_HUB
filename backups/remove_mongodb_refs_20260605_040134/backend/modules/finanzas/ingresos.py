...
MAYO 2026 - SUBFASE 2.4:
- Lee datos REALES desde EDARSAHUB.Finanzas_CortesCaja
- NO usa datos demo por defecto
- NO depende de conexiones en vivo a SoftRestaurant/MPRO
- Filtro obligatorio: EsDemo = 0
- Respeta RBAC por UnidadNegocioID
...
import secrets  # Para datos demo legacy

# Importar repositorio EDARSAHUB
from modules.finanzas.repository_ingresos_edarsahub import (
...
async def get_user_unidades_permitidas(current_user: Dict[str, Any]) -> List[str]:
    """
    RBAC: Obtiene los UUIDs de UnidadNegocioID permitidas para el usuario.
    Retorna lista vacía si el usuario tiene acceso total (admin).
    Lee desde EDARSAHUB para mapear empresas a unidades.
    """
    from modules.finanzas.repository_ingresos_edarsahub import get_edarsahub_connection
    
    empresas_permitidas = await get_user_empresas_permitidas(current_user)
    if not empresas_permitidas:
        return []  # Sin restricción (admin) - acceso a todas las unidades
    
    # Obtener IDs de unidades desde EDARSAHUB que correspondan a las empresas permitidas
    try:
        conn = get_edarsahub_connection()
        cursor = conn.cursor(as_dict=True)
        
        # Buscar unidades que matcheen con los IDs de empresa permitidos
        placeholders = ','.join(['%s'] * len(empresas_permitidas))
        cursor.execute(f"""
            SELECT CAST(id AS NVARCHAR(100)) as unidad_negocio_id
            FROM Unidades_Negocio
            WHERE activo = 1
              AND (
                CAST(id AS NVARCHAR(100)) IN ({placeholders})
                OR empresa_id IN ({placeholders})
              )
        """, tuple(empresas_permitidas) + tuple(empresas_permitidas))
        
        unidades = [str(row['unidad_negocio_id']) for row in cursor.fetchall()]
        conn.close()
        
        return unidades
        
    except Exception as e:
        logger.warning(f"[INGRESOS] Error obteniendo unidades permitidas: {e}")
        return []
...
async def get_user_sucursales_permitidas(current_user: Dict[str, Any]) -> List[str]:
    """
    DEPRECATED: Usar get_user_unidades_permitidas en su lugar.
    Mantiene compatibilidad con código legacy.
    """
    return await get_user_unidades_permitidas(current_user)
...
def generar_cortes_caja_demo():
    ...
    for dias_atras in range(30):  # Últimos 30 días
        ...
        for sucursal in sucursales:
            ...
            efectivo = round(demo_uniform(8000, 45000), 2)
            debito = round(demo_uniform(15000, 65000), 2)
            credito = round(demo_uniform(10000, 50000), 2)
            amex = round(demo_uniform(2000, 15000), 2) if demo_random() > 0.3 else 0
            internacional = round(demo_uniform(1000, 8000), 2) if demo_random() > 0.5 else 0
            ...
            # Estado de depósito
            hoy = datetime.now()
            efectivo_depositado = fecha_dep_efectivo.date() < hoy.date() and demo_random() > 0.2
            tarjetas_depositadas = fecha_dep_debito.date() < hoy.date() and demo_random() > 0.15
            ...
            corte = {
                ...
                "efectivo_referencia_deposito": f"DEP-{demo_randint(10000, 99999)}" if efectivo_depositado else None,
                ...
                "tarjetas_referencia_netpay": f"NP-{demo_randint(100000, 999999)}" if tarjetas_depositadas else None,
...
async def listar_cortes_caja(
    unidad_negocio_id: Optional[str] = Query(None, description="UUID de la unidad de negocio"),
    sucursal_id: Optional[int] = Query(None, description="DEPRECATED: Usar unidad_negocio_id"),
    fecha_inicio: Optional[str] = Query(None, description="Fecha inicio YYYY-MM-DD"),
    fecha_fin: Optional[str] = Query(None, description="Fecha fin YYYY-MM-DD"),
    sistema_origen: Optional[str] = Query(None, description="SoftRestaurant, MPRO, o None para todos"),
    solo_pendientes: bool = Query(False, description="Solo cortes pendientes de depósito"),
    use_demo: bool = Query(False, description="DEPRECATED: Usar datos demo (no recomendado)"),
    current_user: Dict = Depends(get_current_user)
):
    """
    Listar cortes de caja con detalle de ingresos.
    
    SUBFASE 2.4 - Lee desde EDARSAHUB (datos sincronizados)
    - Fuente: Finanzas_CortesCaja con EsDemo=0
    - NO depende de conexiones en vivo a SoftRestaurant/MPRO
    - Respeta RBAC por UnidadNegocioID
    """
    try:
        ...
        # Si se especifica unidad_negocio_id, validar que esté permitida
        if unidad_negocio_id and unidades_permitidas:
        ...
        if not cortes:
            return {
                ...
                "total": 0,
                "fuente": "EDARSAHUB_REAL",
                "mensaje": "Sin cortes sincronizados para el rango seleccionado. Verifique que los datos estén sincronizados desde SoftRestaurant/MPRO.",
                "resumen": {
                    "total_efectivo": 0,
                    "total_tarjetas_bruto": 0,
                    "total_comisiones": 0,
        ...
        # Calcular totales
        total_efectivo = sum(c.get("efectivo", 0) for c in cortes)
        total_tarjetas = sum(
            c.get("debito", 0) + c.get("credito", 0) + 
            c.get("amex", 0) + c.get("internacional", 0) 
            for c in cortes
        )
        total_comisiones = sum(c.get("total_comisiones", 0) for c in cortes)
        total_neto = sum(c.get("total_neto_tarjetas", 0) for c in cortes)
        
        return {
            "cortes": cortes[:100],  # Limitar a 100 para performance
            "total": len(cortes),
            "fuente": "EDARSAHUB_REAL",
            "resumen": {
...
async def _listar_cortes_demo(
    ...
    """
    DEPRECATED: Función legacy para datos demo.
    Solo se usa si explícitamente se pide use_demo=true y hay error.
    """
    ...
    return {
        ...
        "total": len(cortes),
        "fuente": "DEMO_LEGACY",
        "mensaje": "ADVERTENCIA: Usando datos demo legacy. Los datos reales no están disponibles.",
        "resumen": {
            "total_efectivo": round(total_efectivo, 2),
            "total_tarjetas_bruto": round(total_tarjetas, 2),
            "total_comisiones": round(total_comisiones, 2),
