...
class DateFilterPolicy:
    ...
    @staticmethod
    def build_sql_date_filter_safe(
        column_name: str,
        fecha_ini: str,
        fecha_fin: str,
        ...
        """
        Construye un filtro SQL SEGURO para rango de fechas usando CONVERT.
        
        USAR ESTE MÉTODO para queries a servidores SQL externos donde
        la configuración regional puede variar.
        
        Args:
            column_name: Nombre de la columna SQL (ej: 'Vn_Fecha')
            fecha_ini: Fecha inicio en cualquier formato
            fecha_fin: Fecha fin en cualquier formato
            include_time: Si True, incluye tiempo hasta 23:59:59
            
        Returns:
            String SQL con CONVERT para compatibilidad regional
            
        Example:
            >>> DateFilterPolicy.build_sql_date_filter_safe('Vn_Fecha', '2026-04-01', '2026-04-21')
            "CONVERT(varchar, Vn_Fecha, 112) >= '20260401' AND CONVERT(varchar, Vn_Fecha, 112) <= '20260421'"
        """
        fi = DateFilterPolicy.to_yyyymmdd(fecha_ini)
        ff = DateFilterPolicy.to_yyyymmdd(fecha_fin)
        
        col_converted = f"CONVERT(varchar, {column_name}, 112)"
        return f"{col_converted} >= '{fi}' AND {col_converted} <= '{ff}'"
    
    @staticmethod
    def build_sql_date_equals_safe(column_name: str, fecha: str) -> str:
        """
        Construye un filtro SQL SEGURO para igualdad de fecha.
        ...
        Returns:
            String SQL con CONVERT para comparación segura
        """
        f = DateFilterPolicy.to_yyyymmdd(fecha)
        return f"CONVERT(varchar, {column_name}, 112) = '{f}'"
...
build_sql_date_filter_safe = DateFilterPolicy.build_sql_date_filter_safe
build_sql_date_equals_safe = DateFilterPolicy.build_sql_date_equals_safe
