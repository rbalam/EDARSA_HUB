...
TARGETS = [
    ROOT / "modules/crm",
    ROOT / "modules/tablajeria",
    ROOT / "modules/rh",
    ROOT / "modules/inteligencia_comercial",
    ROOT / "modules/api_connections",
    ROOT / "core/scheduler",
    ROOT / "tools",
]
...
for base in TARGETS:
    ...
    for p in base.rglob("*.py"):
        ...
        if updated != original:
            ...
            backup = p.with_suffix(p.suffix + f".bak_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
if skipped:
    print(f"TOTAL SIN CAMBIO AUTOMÁTICO: {len(skipped)}")
