...
# Unidades de negocio con sus configuraciones
UNIDADES_CONFIG = {
    'CIENFUEGOS': {
...
def _get_unidad_config(unidad_negocio_id: str) -> Optional[Dict[str, Any]]:
    """Obtiene configuración de la unidad."""
    return UNIDADES_CONFIG.get(unidad_negocio_id)
...
async def validar_resync(
    ...
    if not unidad:
        raise HTTPException(
            status_code=400, 
            detail=f"Unidad '{request.unidad_negocio_id}' no encontrada. Disponibles: {list(UNIDADES_CONFIG.keys())}"
        )
...
async def obtener_opciones_resync(
    ...
    return {
        ...
        'unidades': [
            ...
            for k, v in UNIDADES_CONFIG.items()
        ]
    }
