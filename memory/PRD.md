...
- [ ] Eliminar 28 colecciones Mongo restantes (pendiente script usuario)
  ...
  - [x] (2026-06-07) LISTA BLANCA EDITABLE (Fase 4 NO destructiva): de las 107 colecciones, **31 candidatas editables** (24 `test_database` BASE_DE_PRUEBAS + 7 vacías CANDIDATA_A_BORRADO_POSTERIOR) y **76 excluidas por seguridad** (sensibles: users/auth/rbac/logs/inventarios/config/finanzas/comercial, o recomendación no permitida). CSV editable con `aprobar_borrado=NO` por defecto. Salidas en `/app/docs/reports/mongo_delete_whitelist_20260607_183030/` (CSV/MD/JSON). Script usuario AUDITADO (NO destructivo, solo lee JSON Fase 3 + escribe reportes) y ejecutado equivalente. PENDIENTE: usuario marca `SI` en CSV → script de borrado quirúrgico futuro (con re-respaldo previo) leerá solo filas aprobadas.
