...
- [ ] Eliminar 28 colecciones Mongo restantes (pendiente script usuario)
  ...
  - [x] (2026-06-07) REGISTRO SQL DE AUDITORÍA (Fase 5 NO destructiva): creada `dbo.Sistema_MongoSunset_Auditoria` (idempotente) en EDARSAHUB con las 31 filas de la lista blanca como `PENDIENTE_AUTORIZACION` / `AprobarBorrado=NO` (24 BASE_DE_PRUEBAS + 7 CANDIDATA_A_BORRADO_POSTERIOR). Trazabilidad permanente del sunset. Persistencia verificada con conexión independiente. Reportes en `/app/docs/reports/mongo_sunset_sql_audit_20260607_185309/`. Auditoría de viabilidad: usuario SQL `HRLectura` es realmente `db_owner` (CREATE/INSERT OK); pool con `autocommit=True` → writes persisten vía execute_sql_query. Script usuario AUDITADO (usaba pyodbc innecesario como fallback nunca alcanzado; reimplementado con execute_sql_query, mismo esquema de tabla). NADA borrado.
