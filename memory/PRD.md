...
- [ ] Eliminar 28 colecciones Mongo restantes (pendiente script usuario)
  ...
  - [x] (2026-06-07) VERIFICACIÓN DE COBERTURA CANÓNICA (Fase 6-bis, SOLO LECTURA): 34 colecciones sensibles (MIGRAR/VALIDAR) comparadas contra 478 tablas EDARSAHUB con hints canónicos reales. Dictamen: 27 CUBIERTA_PARCIALMENTE (validar muestra/llaves antes de borrar) + 7 SIN_COBERTURA (requieren migración semántica real, NO borrar). Matches fuertes confirmados: rbac_roles→Sistema_RBAC_Roles (6=6), inventarios_procesados_auto→Inventarios_ProcesadosAuto (158=158), portal_suppliers→Portal_Proveedores (3=3), sec_unidades_negocio→Unidades_Negocio. Críticas SIN cobertura: rbac_audit_log (598 docs), sec_bitacora_admin (61), sec_bitacora_acceso (27) → bitácoras sin tabla canónica equivalente. Reportes en `/app/docs/reports/mongo_cobertura_canonica_20260607_190521/`. Reemplaza la Fase 6 (archivo JSON) que se RECHAZÓ por redundante. NADA borrado/modificado.
