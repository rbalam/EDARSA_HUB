...
- ✅ **Centralización modales de detalle + selector inventarios (P1):** 3 módulos canónicos compartidos entre Análisis (`Reportes.js`) y Auditoría (`Compras.js`): `DetalleProductoModal.jsx` (modal único movimientos/consumos), `useDetalleProducto.js` (hook que consulta SIEMPRE los endpoints canónicos `/compras/detalle-*`), `inventarioSelectorUtils.js` (fecha mínima + filtrado de finales). Análisis deja de usar `/reports/*` (que fallaban). Verificado: webpack OK, eslint limpio, shapes por curl, utils por node. Test visual e2e bloqueado por reset de caché del preview (limitación de entorno, no del app).
